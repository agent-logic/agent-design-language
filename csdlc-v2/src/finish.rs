...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "snake_case")]
pub enum CachedTerminalDiagnosticStatus {
    SameProjectionTerminal,
    StaleProjectionTerminalExists,
    ConflictingTerminalAuthority,
    MissingTerminalAuthority,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
pub struct LocalProjectionDiagnostic {
    pub issue: u64,
    pub repository: String,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
pub struct CachedTerminalDiagnostic {
    pub schema: String,
    pub issue: u64,
    pub status: CachedTerminalDiagnosticStatus,
    pub local_projection: LocalProjectionDiagnostic,
    pub terminal_cache_path: String,
    pub terminal: Option<DerivedTerminalEnvelope>,
    pub canonical_match: bool,
    pub immutable_receipt_overwrite_allowed: bool,
    pub next_action: String,
}

pub fn diagnose_cached_terminal(root: &Path, issue: u64) -> Result<CachedTerminalDiagnostic> {
    let store = Store::new(root);
    let record = store.load_record(issue)?;
    let terminal_cache_path = terminal_cache_path(root, issue)?;
    let local_projection = LocalProjectionDiagnostic {
        issue: record.issue,
    ...
    let terminal = load_cached_terminal(root, issue)?;
    let Some(terminal) = terminal else {
        return Ok(cached_terminal_diagnostic(
            issue,
            CachedTerminalDiagnosticStatus::MissingTerminalAuthority,
            local_projection,
            terminal_cache_path,
            None,
            false,
            "run finish from the canonical issue worktree after live GitHub terminal evidence is available",
        ));
    };
    let canonical_match = envelope_matches_record_in_repo(root, &terminal, &record)?;
    let status = if canonical_match {
        CachedTerminalDiagnosticStatus::SameProjectionTerminal
    } else if terminal.issue == record.issue
        && terminal.repository == record.repository
        && terminal.initialization_digest == record.initialization_digest
        && terminal.canonical_generation > record.generation
    {
        CachedTerminalDiagnosticStatus::StaleProjectionTerminalExists
    } else {
        CachedTerminalDiagnosticStatus::ConflictingTerminalAuthority
    };
    let next_action = match status {
        CachedTerminalDiagnosticStatus::SameProjectionTerminal => {
            "cached terminal authority matches this projection; proceed with normal validation or cleanup gates"
        }
        CachedTerminalDiagnosticStatus::StaleProjectionTerminalExists => {
            "this worktree projection is stale; switch to the worktree/revision that produced the cached terminal generation before cleanup or closeout"
        }
        CachedTerminalDiagnosticStatus::ConflictingTerminalAuthority => {
            "stop and inspect terminal receipt lineage; cached terminal authority conflicts with this projection"
        }
        CachedTerminalDiagnosticStatus::MissingTerminalAuthority => unreachable!("handled above"),
    };
    Ok(cached_terminal_diagnostic(
        issue,
        status,
        local_projection,
        terminal_cache_path,
...
fn cached_terminal_diagnostic(
    issue: u64,
    status: CachedTerminalDiagnosticStatus,
    local_projection: LocalProjectionDiagnostic,
    terminal_cache_path: PathBuf,
    terminal: Option<DerivedTerminalEnvelope>,
    canonical_match: bool,
    next_action: &str,
) -> CachedTerminalDiagnostic {
    CachedTerminalDiagnostic {
        schema: "csdlc.cached_terminal_diagnostic.v1".into(),
        issue,
        status,
        local_projection,
        terminal_cache_path: terminal_cache_path.display().to_string(),
        terminal,
        canonical_match,
        immutable_receipt_overwrite_allowed: false,
