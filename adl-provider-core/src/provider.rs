...
fn invalid_config(provider: &str, message: impl Into<String>) -> anyhow::Error {
    ...
    let mut error = ProviderError::invalid_config(provider, message.clone());
    if message.starts_with("missing required auth env var")
        || message.starts_with("missing Vertex AI bearer token")
        || message.starts_with("Vertex AI bearer token env var")
        || message.starts_with("ADC token acquisition")
    {
...
/// Sanitized adapter-owned failure classification; never includes response or credentials.
pub fn failure_category(err: &anyhow::Error) -> &'static str {
    for cause in err.chain() {
...
fn http_status_error(provider: &str, status: reqwest::StatusCode) -> anyhow::Error {
    ...
    let message = format!("kind={class} status={status} body=<redacted>");
    let mut error = if status.is_client_error() {
        ProviderError::runtime_non_retryable(provider, message)
    } else {
        ProviderError::runtime(provider, message)
    };
    error.category = match status.as_u16() {
        401 | 403 => "credentials",
        429 => "quota",
        404 => "model_unavailable",
        400 | 405 | 422 => "invalid_configuration",
        _ => "transport",
    };
    error.into()
}
