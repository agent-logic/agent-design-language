...
pub fn provider_profile_materialization_projection(doc: &ProviderMap) -> Result<Value> {
    let expanded = expand_provider_profiles(doc)?;
    let mut providers = Map::new();
    let mut provider_ids: Vec<_> = expanded.keys().cloned().collect();
    provider_ids.sort();

    for provider_id in provider_ids {
        let spec = &expanded[&provider_id];
...
fn materialized_active_provider_profiles(doc: &ProviderMap) -> Result<ProviderMap> {
    let registry = provider_profile_registry();
    let active = doc.clone();
    let mut provider_ids: Vec<String> = active.keys().cloned().collect();
    provider_ids.sort();

    for provider_id in provider_ids {
        let Some(spec) = active.get(&provider_id) else {
            continue;
        };
...
pub fn expand_provider_profiles(doc: &ProviderMap) -> Result<ProviderMap> {
    let registry = provider_profile_registry();
    let available = provider_profile_names().join(", ");
    let mut expanded = doc.clone();
    let mut provider_ids: Vec<String> = expanded.keys().cloned().collect();
    provider_ids.sort();

    for provider_id in provider_ids {
        let Some(spec) = expanded.get(&provider_id).cloned() else {
            continue;
        };
...
pub fn activate_provider_profile_candidate(
    active_doc: &ProviderMap,
    candidate_doc: &ProviderMap,
...
fn promote_last_known_good_materializations(doc: &mut ProviderMap) -> Result<()> {
    let registry = provider_profile_registry();
    let mut provider_ids: Vec<String> = doc.keys().cloned().collect();
    provider_ids.sort();
    for provider_id in provider_ids {
        let Some(spec) = doc.get_mut(&provider_id) else {
            continue;
        };
