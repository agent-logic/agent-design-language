...
mod tests {
    ...
    #[test]
    fn full_review_inventory_and_negative_fixtures() {
        let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../../../../..");
        let result = Command::new("python3")
            .current_dir(root)
            .args([
                "docs/milestones/v0.92.2/evidence/issue-919/validate_full_review.py",
                "--self-test",
            ])
            .output()
            .expect("start full review packet validator");
        assert!(
            result.status.success(),
            "full review proof failed:\n{}\n{}",
        ...
        assert!(stdout.contains("\"qualification_pass\": false"));
    }
