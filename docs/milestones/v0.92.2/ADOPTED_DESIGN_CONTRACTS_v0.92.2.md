...
The operator revised the Beta 1 boundary under #945: an integrated invitation-only website with GitHub sign-in must support both server-hosted reviews and website-controlled reviews executed by an installed local agent, with CLI support also retained. Agent Logic provides model access for server-hosted reviews. [ADR 0084](../../adr/0084-cf-09.md) and [the delivery gates](adr/issue-945/BETA1_DELIVERY.md) supersede the original CLI-only selection. Shared Runtime/provider and Memory Palace ownership remain. Website and private code repositories are separate; the code repository name remains undecided. v0.93 extraction does not defer Beta 1 website functionality. Missing component ownership and typed #914/#915 dependency rebaselining remain prerequisites to execution; this decision does not claim deployment or qualification. The Observatory static sidecar remains separately owned.
