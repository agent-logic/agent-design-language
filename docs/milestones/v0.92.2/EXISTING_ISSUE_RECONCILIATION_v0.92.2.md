...
The denominator is 43 rows: the original 30 planning rows, newly admitted PLAT-PAIR and OPS-GCP, one reused existing issue, and ten SIM sprint rows. Resolve WP-01's conductor separately; it is not its own creation target. Then reconcile 42 remaining rows: 41 prospective creations (requiring separate authorization) and one existing issue.
