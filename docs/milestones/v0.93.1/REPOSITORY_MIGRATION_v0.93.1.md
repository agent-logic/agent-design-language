# v0.93.1 Opening Repository Migration
...
Public ADL builds/tests/packages with no private registry credentials. Each private product installs and releases from its own checkout without sibling source. Runtime runs continuity/API/Observatory proof; CodeFriend demonstrates clean-checkout installation and parity with the frozen baseline on ADL and an external repository, preserving failed, unknown and unsupported #915 cells. RD-11 proves portability and preserved known limitations, not completion of future CF/CT features or Runtime v4. Missing launch behavior is then delivered and qualified through CF-01–CF-07 and #1148–#1150 after RD-11; it cannot circularly block the start of that work. Infrastructure validates pinned artifacts without an implicit cloud apply. Website behavior is compared before/after and its deployment authority is preserved.
...
## Dependency reduction acceptance

The split is the next major result because the combined repository couples too many owners and dependencies. Each destination must independently build, test, install and release using explicit distributable versions, without sibling checkouts or implicit Runtime/C-SDLC source coupling. Keep shared contracts small and stable, assign one implementation owner, and accept one cross-product lockset. Existing cutover proof carries these checks; do not add another ceremony or approval chain. A directory move with the same hidden coupling fails RD-11.
