...
| Runtime first-birthday demo: `.csdlc/issues/5836` and `docs/milestones/v0.92/features/FIRST_BIRTHDAY_DEMO_AND_GOVERNANCE_HANDOFF_v0.92.md` | `blocked_with_evidence`: #5836 is reviewed locally, but final publication remains gated by Observatory/operator evidence. | Use only the declared demo contract and blocker status as planning context until #5836 terminal evidence is available. | Must not claim the birthday demo is released, public, governance-complete, or externally publishable from this map. | Use documented packet fields and blocker classification; do not expose unreviewed local runtime artifacts. | v0.93 must wait for terminal v0.92 demo evidence before treating demo outputs as governance inputs. | v0.93 flagship constitutional-review demo planning. |
