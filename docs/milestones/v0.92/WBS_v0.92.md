...
| WP-02 | Agent Logic GitHub organization repository copy | Create five independent `agent-logic` destination mirrors before substantive milestone work while retaining all seven `danielbaustin` sources unchanged and excluding `asksifu` and Horust from copying. | Verified company-owned organization, five serial destination copies with four private and ADL public, exact Git/LFS parity, destination-only configuration dispositions, source-immutability proof, and final copy report. | WP-01, WP-01B, superseding copy-only plan, confirmed destination organization and owners, complete seven-source inventory, billing/security readiness. |
...
| WP-18A | Observatory/Unity consumer integration — deferred from v0.92 | No v0.92 implementation claim. Backlog #84 owns Unity integration; #122 is scheduled for v0.92.1 and #251 remains backlog. | Explicit milestone scope disposition with no synthetic consumer proof. | Later execution under #84 after #122 and #251. |
...
| WP-20 | Demo matrix, AEE proof, and proof coverage | Align demos and AEE proof expectations with milestone claims. | Demo matrix, AEE proof routing or packet, commands, artifacts, and validation notes. | WP-18, WP-18B, WP-19; WP-18A is explicitly scoped out. |
...
1. Update canonical docs, complete the five repository copies, repair CI/coverage,
