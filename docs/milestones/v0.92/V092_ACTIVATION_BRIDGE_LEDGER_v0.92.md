...
| Reasoning graph, loop runtime, and `adl.skill.v1` | Runtime v3 loop prerequisite requalified; broader bridge integration_required | `v0.91.7` WP-11, historical `#5104` merge evidence, and `.csdlc/evidence/5817/prerequisite-and-loop-runtime-requalification.md` | WP-01 confirmed that current Runtime v3 implements bounded execution, graph/state identity, deterministic replay, cancellation, resume continuity, forgery rejection, and supervised startup. Prompts, skills, trace, ObsMem, PVF, AEE, UTS, ACC, and `adl.skill.v1` convergence remains owned by its v0.92 feature issues. |
| Adaptive Learning DAG | implementation_required | `v0.92` WP-13A after WP-01 Runtime v3 loop requalification | WP-13A must implement evaluation bindings, stateful adaptation, policy-governed graph modification, accepted/rejected mutation evidence, and integrated replay proof. Bounded loop proof alone does not establish learning-driven graph mutation. |
...
- Adaptive Learning DAG implementation after verified loop-runtime evidence
- security readiness
