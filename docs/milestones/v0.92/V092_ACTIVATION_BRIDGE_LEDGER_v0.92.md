...
| Reasoning graph, loop runtime, and `adl.skill.v1` | implementation_required | `v0.91.7` WP-11, including `#5104` merge evidence for loop runtime | Pre-`v0.92` implementation must connect prompts, skills, loops, trace, ObsMem, PVF, AEE, Runtime v2, UTS, ACC, and `adl.skill.v1`; deeper convergence remains later. WP-01 must verify whether `#5104` makes loops validated, replayable Runtime v2 objects before consuming that claim. |
