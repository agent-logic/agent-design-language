...
ISSUE = 163
WORK_PACKAGE = "V3-D11"
REQUIRED_ARTIFACTS = ["docs/adr/ADR-CSDLC-V3-PLATFORM-COMMIT-MATRIX.md", "csdlc-v3/contracts/platform-commit-matrix.v1.json"].freeze
REQUIRED_LANES = {"v3-d11-diff-hygiene"=>["git", "diff", "--check", "origin/main...HEAD"]}.freeze
EXPECTED_OBSERVATIONS = {"unmeasured_supported_platform_count"=>["eq", 0], "ambiguous_platform_posture_count"=>["eq", 0], "authorized_operator_decision_count"=>["eq", 1], "windows_posture_valid"=>["eq", true], "v3_08_without_decision_rejected"=>["eq", true]}.freeze
PROOF_PATH = File.join(ROOT, ".csdlc/evidence/163/proof.json")

def fail!(message)
  abort("FAIL [V3-D11]: " + message)
end
...
puts "PASS: V3-D11 exact-head artifact, producer, and invariant proof"
