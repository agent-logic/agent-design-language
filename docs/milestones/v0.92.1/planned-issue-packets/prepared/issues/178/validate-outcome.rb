...
ISSUE = 178
WORK_PACKAGE = "V3-15"
REQUIRED_ARTIFACTS = ["csdlc-v3/src/commands/finish/mod.rs", "csdlc-v3/src/commands/finish/reconcile.rs", "csdlc-v3/src/commands/clean/mod.rs", "csdlc-v3/src/commands/clean/classify.rs", "csdlc-v3/tests/terminal/linkage_finish.rs", "csdlc-v3/tests/terminal/checkpoints.rs", "csdlc-v3/tests/terminal/cleanup_paths.rs", "csdlc-v3/tests/terminal/cleanup_eligibility.rs"].freeze
REQUIRED_LANES = {"v3-15-focused-rust"=>["cargo", "test", "--locked", "--manifest-path", "csdlc-v3/Cargo.toml", "--all-targets"], "v3-15-diff-hygiene"=>["git", "diff", "--check", "origin/main...HEAD"]}.freeze
EXPECTED_OBSERVATIONS = {"ambiguous_pr_terminal_acceptance_count"=>["eq", 0], "part_of_parent_terminal_completion_count"=>["eq", 0], "checkpoint_authorization_survivor_count"=>["eq", 0], "external_parent_close_checkpoint_credit_count"=>["eq", 0], "cleanup_default_destructive_count"=>["eq", 0], "noncanonical_path_cleanup_count"=>["eq", 0], "other_worktree_deletion_target_count"=>["eq", 0], "cleanup_without_closed_out_receipt_count"=>["eq", 0]}.freeze
PROOF_PATH = File.join(ROOT, ".csdlc/evidence/178/proof.json")

def fail!(message)
  abort("FAIL [V3-15]: " + message)
end
...
puts "PASS: V3-15 exact-head artifact, producer, and invariant proof"
