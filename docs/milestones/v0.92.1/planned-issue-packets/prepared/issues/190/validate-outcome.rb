...
issue = 190
manifest_path = File.join(root, ".csdlc/evidence/190/proof-manifest.json")
...
payload = read_json(artifact_path(manifest, "handoff"))
abort("wrong proof schema") unless payload["schema"] == "adl.v0921.int03.proof.v1"
abort("proof revision mismatch") unless payload["revision"] == revision

terminal = payload.fetch("terminal_release")
abort("release closeout is not terminal") unless terminal["terminal"] == true && git_sha?(terminal["revision"]) && sha?(terminal["evidence_sha256"])
risks = payload.fetch("residual_risks")
abort("residual risk lacks owner/disposition") unless risks.all? { |r| present?(r["id"]) && present?(r["owner"]) && present?(r["disposition"]) }
deferred = payload.fetch("deferred_work")
abort("deferred identifiers not unique") unless deferred.map { |d| d.fetch("id") }.uniq.length == deferred.length
abort("deferred work incomplete") unless deferred.all? { |d| present?(d["owner"]) && !d.fetch("dependencies").empty? && !d.fetch("proof_requirements").empty? && present?(d["route"]) && d["complete"] == false }
eligibility = payload.fetch("v3_r01_eligibility")
required = %w[rollback_window_expired stability_metrics historical_readability operator_approval]
abort("V3-R01 eligibility gate denominator mismatch") unless eligibility.fetch("gates").keys.sort == required.sort
computed = eligibility.fetch("gates").values.all? { |gate| gate["satisfied"] == true && present?(gate["evidence_sha256"]) }
abort("V3-R01 eligibility result not recomputed") unless eligibility["eligible"] == computed
paths = payload.fetch("references")
abort("handoff contains machine-local reference") if paths.any? { |p| p.start_with?("/") || p.include?(".worktree") || p.include?("/tmp/") }
abort("handoff reference missing") unless paths.all? { |p| File.exist?(File.join(root, p)) }

puts "PASS: issue 190 producer artifacts and receipts recomputed at #{revision}"
