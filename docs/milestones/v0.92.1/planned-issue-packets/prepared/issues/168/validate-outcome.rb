...
ISSUE = 168
WORK_PACKAGE = "V3-07"
REQUIRED_ARTIFACTS = ["csdlc-v3/src/lifecycle/mod.rs", "csdlc-v3/src/lifecycle/transition.rs", "csdlc-v3/src/lifecycle/capabilities.rs", "csdlc-v3/tests/lifecycle/exhaustive.rs", "csdlc-v3/tests/lifecycle/recovery.rs", "csdlc-v3/tests/lifecycle/mutation_properties.rs", "csdlc-v3/tests/lifecycle/cleanup_eligibility.rs"].freeze
REQUIRED_LANES = {"v3-07-focused-rust"=>["cargo", "test", "--locked", "--manifest-path", "csdlc-v3/Cargo.toml", "--all-targets"], "v3-07-diff-hygiene"=>["git", "diff", "--check", "origin/main...HEAD"]}.freeze
EXPECTED_OBSERVATIONS = {"unmapped_state_command_pair_count"=>["eq", 0], "ambient_io_call_count"=>["eq", 0], "reachable_typed_correction_missing_count"=>["eq", 0], "merged_recovery_acceptance_count"=>["eq", 0], "closed_out_recovery_acceptance_count"=>["eq", 0], "removed_predicate_surviving_mutation_count"=>["eq", 0], "claim_lease_heartbeat_authority_count"=>["eq", 0]}.freeze
PROOF_PATH = File.join(ROOT, ".csdlc/evidence/168/proof.json")

def fail!(message)
  abort("FAIL [V3-07]: " + message)
end
...
puts "PASS: V3-07 exact-head artifact, producer, and invariant proof"
