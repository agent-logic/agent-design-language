...
ISSUE = 170
WORK_PACKAGE = "V3-09"
REQUIRED_ARTIFACTS = ["csdlc-v3/src/adapters/git.rs", "csdlc-v3/src/adapters/process.rs", "csdlc-v3/src/adapters/credentials.rs", "csdlc-v3/tests/adapters/git.rs", "csdlc-v3/tests/adapters/process.rs", "csdlc-v3/tests/adapters/credentials.rs"].freeze
REQUIRED_LANES = {"v3-09-focused-rust"=>["cargo", "test", "--locked", "--manifest-path", "csdlc-v3/Cargo.toml", "--all-targets"], "v3-09-diff-hygiene"=>["git", "diff", "--check", "origin/main...HEAD"]}.freeze
EXPECTED_OBSERVATIONS = {"shell_evaluation_count"=>["eq", 0], "untyped_process_invocation_count"=>["eq", 0], "collapsed_process_outcome_count"=>["eq", 0], "durable_credential_leak_count"=>["eq", 0], "surviving_child_count"=>["eq", 0], "branch_name_authorization_count"=>["eq", 0]}.freeze
PROOF_PATH = File.join(ROOT, ".csdlc/evidence/170/proof.json")

def fail!(message)
  abort("FAIL [V3-09]: " + message)
end
...
puts "PASS: V3-09 exact-head artifact, producer, and invariant proof"
