...
ISSUE = 180
WORK_PACKAGE = "V3-R01"
REQUIRED_ARTIFACTS = [".csdlc/evidence/180/v2-retirement/deletion-manifest.json", ".csdlc/evidence/180/v2-retirement/eligibility-decision.json", ".csdlc/evidence/180/v2-retirement/retained-evidence-inventory.json", ".csdlc/evidence/180/v2-retirement/install-inventory.json", ".csdlc/evidence/180/v2-retirement/fresh-checkout-journey.json", ".csdlc/evidence/180/v2-retirement/final-authority-audit.json"].freeze
REQUIRED_LANES = {"v3-r01-diff-hygiene"=>["git", "diff", "--check", "origin/main...HEAD"]}.freeze
EXPECTED_OBSERVATIONS = {"unclassified_deletion_target_count"=>["eq", 0], "historical_evidence_deletion_count"=>["eq", 0], "remaining_v2_executable_count"=>["eq", 0], "remaining_v2_operator_skill_count"=>["eq", 0], "remaining_v2_selector_route_count"=>["eq", 0], "remaining_v2_writable_authority_count"=>["eq", 0], "fresh_v3_only_journey_failure_count"=>["eq", 0]}.freeze
PROOF_PATH = File.join(ROOT, ".csdlc/evidence/180/proof.json")

def fail!(message)
  abort("FAIL [V3-R01]: " + message)
end
...
puts "PASS: V3-R01 exact-head artifact, producer, and invariant proof"
