...
ISSUE = 162
WORK_PACKAGE = "V3-02"
REQUIRED_ARTIFACTS = ["csdlc-v3/spike/Cargo.toml", "csdlc-v3/spike/src/lib.rs", "csdlc-v3/spike/tests/construction_slice.rs", "csdlc-v3/evidence/construction-spike/thresholds.json", "csdlc-v3/evidence/construction-spike/dependency-inventory.json", "csdlc-v3/evidence/construction-spike/github-capability-gaps.json", "csdlc-v3/evidence/construction-spike/platform-commit-recommendation.json", "csdlc-v3/evidence/construction-spike/disposition.json"].freeze
REQUIRED_LANES = {"v3-02-focused-rust"=>["cargo", "test", "--locked", "--manifest-path", "csdlc-v3/Cargo.toml", "--all-targets"], "v3-02-diff-hygiene"=>["git", "diff", "--check", "origin/main...HEAD"]}.freeze
EXPECTED_OBSERVATIONS = {"binary_target_count"=>["eq", 1], "library_target_count"=>["eq", 1], "missing_measurement_count"=>["eq", 0], "threshold_miss_count"=>["eq", 0], "raw_github_operation_count"=>["lte", 3], "unexpected_fake_operation_rejections"=>["gte", 1], "real_state_mutation_count"=>["eq", 0], "decision11_issued"=>["eq", false]}.freeze
PROOF_PATH = File.join(ROOT, ".csdlc/evidence/162/proof.json")

def fail!(message)
  abort("FAIL [V3-02]: " + message)
end
...
puts "PASS: V3-02 exact-head artifact, producer, and invariant proof"
