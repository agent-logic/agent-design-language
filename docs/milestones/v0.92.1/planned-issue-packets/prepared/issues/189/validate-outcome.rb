...
issue = 189
manifest_path = File.join(root, ".csdlc/evidence/189/proof-manifest.json")
...
payload = read_json(artifact_path(manifest, "release_candidate"))
abort("wrong proof schema") unless payload["schema"] == "adl.v0921.int02.proof.v1"
abort("proof revision mismatch") unless payload["revision"] == revision

candidate = payload.fetch("candidate")
abort("candidate source revision invalid") unless git_sha?(candidate["source_revision"])
abort("candidate manifest has unpinned entries") unless candidate.fetch("entries").all? { |e| present?(e["id"]) && sha?(e["sha256"]) && present?(e["source_ref"]) }
authority = payload.fetch("authority")
abort("company release authority incomplete") unless present?(authority["release_authority"]) && !authority.fetch("observers").empty? && present?(authority["change_window"]) && !authority.fetch("abort_triggers").empty?
abort("personal credential dependency") unless authority["hidden_personal_credentials"] == false
rehearsal = payload.fetch("rollback_rehearsal")
abort("rollback was not executed from candidate") unless rehearsal["executed"] == true && rehearsal["candidate_sha256"] == digest_file(artifact_path(manifest, "candidate_manifest"))
%w[data_sha256 authority_sha256].each { |field| abort("rollback invariant mismatch #{field}") unless rehearsal.dig("before", field) == rehearsal.dig("after", field) }
abort("dual authority after rollback") unless rehearsal["dual_authority"] == false
abort("rollback cleanup incomplete") unless rehearsal["cleanup_verified"] == true
abort("release action occurred without operator authorization") unless payload["release_action"] == "not_executed" && payload["operator_authorization"] == "pending"

puts "PASS: issue 189 producer artifacts and receipts recomputed at #{revision}"
