...
ISSUE = 166
WORK_PACKAGE = "V3-05"
REQUIRED_ARTIFACTS = ["csdlc-v3/src/repository/mod.rs", "csdlc-v3/src/repository/context.rs", "csdlc-v3/src/import/mod.rs", "csdlc-v3/src/import/v2.rs", "csdlc-v3/tests/import/context_resolution.rs", "csdlc-v3/tests/import/unsupported_fields.rs", "csdlc-v3/tests/import/read_only.rs"].freeze
REQUIRED_LANES = {"v3-05-focused-rust"=>["cargo", "test", "--locked", "--manifest-path", "csdlc-v3/Cargo.toml", "--all-targets"], "v3-05-diff-hygiene"=>["git", "diff", "--check", "origin/main...HEAD"]}.freeze
EXPECTED_OBSERVATIONS = {"ambiguous_identity_success_count"=>["eq", 0], "path_escape_success_count"=>["eq", 0], "silently_dropped_field_count"=>["eq", 0], "unsupported_field_mutation_entry_count"=>["eq", 0], "v2_write_attempt_count"=>["eq", 0], "v3_write_attempt_count"=>["eq", 0]}.freeze
PROOF_PATH = File.join(ROOT, ".csdlc/evidence/166/proof.json")

def fail!(message)
  abort("FAIL [V3-05]: " + message)
end
...
puts "PASS: V3-05 exact-head artifact, producer, and invariant proof"
