...
ISSUE = 171
WORK_PACKAGE = "V3-10A"
REQUIRED_ARTIFACTS = ["csdlc-v3/src/commands/issue/mod.rs", "csdlc-v3/src/commands/bind/mod.rs", "csdlc-v3/tests/commands/issue/initialize.rs", "csdlc-v3/tests/commands/issue/show.rs", "csdlc-v3/tests/commands/bind/topology.rs", "csdlc-v3/tests/commands/bind/collisions.rs"].freeze
REQUIRED_LANES = {"v3-10a-focused-rust"=>["cargo", "test", "--locked", "--manifest-path", "csdlc-v3/Cargo.toml", "--all-targets"], "v3-10a-diff-hygiene"=>["git", "diff", "--check", "origin/main...HEAD"]}.freeze
EXPECTED_OBSERVATIONS = {"direct_flag_typed_input_conflict_acceptance_count"=>["eq", 0], "requested_topology_authorization_count"=>["eq", 0], "branch_name_only_authorization_count"=>["eq", 0], "missing_collision_class_count"=>["eq", 0], "non_idempotent_retry_count"=>["eq", 0], "human_json_outcome_mismatch_count"=>["eq", 0]}.freeze
PROOF_PATH = File.join(ROOT, ".csdlc/evidence/171/proof.json")

def fail!(message)
  abort("FAIL [V3-10A]: " + message)
end
...
puts "PASS: V3-10A exact-head artifact, producer, and invariant proof"
