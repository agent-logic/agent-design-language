...
ISSUE = 177
WORK_PACKAGE = "V3-14"
REQUIRED_ARTIFACTS = ["csdlc-v3/src/adapters/github/write/mod.rs", "csdlc-v3/src/adapters/github/write/pulls.rs", "csdlc-v3/src/commands/pr/mod.rs", "csdlc-v3/src/commands/pr/watch.rs", "csdlc-v3/tests/github/write/idempotency.rs", "csdlc-v3/tests/github/write/linkage.rs", "csdlc-v3/tests/github/write/watch.rs", "csdlc-v3/tests/github/write/cancellation.rs"].freeze
REQUIRED_LANES = {"v3-14-focused-rust"=>["cargo", "test", "--locked", "--manifest-path", "csdlc-v3/Cargo.toml", "--all-targets"], "v3-14-diff-hygiene"=>["git", "diff", "--check", "origin/main...HEAD"]}.freeze
EXPECTED_OBSERVATIONS = {"remote_mutation_before_durable_intent_count"=>["eq", 0], "unverified_mutation_count"=>["eq", 0], "part_of_closed_parent_ready_count"=>["eq", 0], "unqualified_split_repository_linkage_count"=>["eq", 0], "persistent_watch_job_count"=>["eq", 0], "unjoined_watch_task_count"=>["eq", 0], "sleep_past_deadline_count"=>["eq", 0], "implicit_merge_count"=>["eq", 0]}.freeze
PROOF_PATH = File.join(ROOT, ".csdlc/evidence/177/proof.json")

def fail!(message)
  abort("FAIL [V3-14]: " + message)
end
...
puts "PASS: V3-14 exact-head artifact, producer, and invariant proof"
