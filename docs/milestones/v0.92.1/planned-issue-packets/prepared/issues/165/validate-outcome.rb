...
ISSUE = 165
WORK_PACKAGE = "V3-04"
REQUIRED_ARTIFACTS = ["csdlc-v3/src/app/mod.rs", "csdlc-v3/src/app/services.rs", "csdlc-v3/src/app/lazy.rs", "csdlc-v3/src/error/mod.rs", "csdlc-v3/src/cancel/mod.rs", "csdlc-v3/tests/app/lazy_services.rs", "csdlc-v3/tests/app/cancellation.rs", "csdlc-v3/tests/app/output_redaction.rs"].freeze
REQUIRED_LANES = {"v3-04-focused-rust"=>["cargo", "test", "--locked", "--manifest-path", "csdlc-v3/Cargo.toml", "--all-targets"], "v3-04-diff-hygiene"=>["git", "diff", "--check", "origin/main...HEAD"]}.freeze
EXPECTED_OBSERVATIONS = {"mutable_global_service_locator_count"=>["eq", 0], "eager_credential_service_initializations"=>["eq", 0], "sync_initialization_count_under_contention"=>["eq", 1], "async_retry_after_leader_drop_count"=>["eq", 1], "deadlocked_waiter_count"=>["eq", 0], "surviving_child_or_task_count"=>["eq", 0], "stdout_diagnostic_mix_count"=>["eq", 0], "durable_secret_or_local_path_count"=>["eq", 0]}.freeze
PROOF_PATH = File.join(ROOT, ".csdlc/evidence/165/proof.json")

def fail!(message)
  abort("FAIL [V3-04]: " + message)
end
...
puts "PASS: V3-04 exact-head artifact, producer, and invariant proof"
