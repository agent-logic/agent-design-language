...
ISSUE = 161
WORK_PACKAGE = "V3-01"
REQUIRED_ARTIFACTS = ["csdlc-v3/contracts/product-contract.v1.json", "csdlc-v3/contracts/retained-v2-invariants.v1.json", "csdlc-v3/contracts/capability-matrix.v1.json", "csdlc-v3/contracts/publication-linkage.v1.json", "csdlc-v3/contracts/state-size-baseline.v1.json", "csdlc-v3/contracts/supported-jq-subset.v1.json", "csdlc-v3/contracts/dependency-manifest.v1.json", "csdlc-v3/schemas/output-envelope.v1.schema.json", "csdlc-v3/tests/contracts/product_contract.rs"].freeze
REQUIRED_LANES = {"v3-01-focused-rust"=>["cargo", "test", "--locked", "--manifest-path", "csdlc-v3/Cargo.toml", "--all-targets"], "v3-01-diff-hygiene"=>["git", "diff", "--check", "origin/main...HEAD"]}.freeze
EXPECTED_OBSERVATIONS = {"unowned_invariant_count"=>["eq", 0], "unowned_mutable_field_count"=>["eq", 0], "state_block_baseline_multiple"=>["gte", 10], "warning_percent"=>["eq", 80], "max_canary_percent_of_block"=>["lte", 50], "external_filter_process_count"=>["eq", 0], "unsupported_jq_typed_error_cases"=>["gte", 1], "official_cli_blob_mismatch_count"=>["eq", 0]}.freeze
PROOF_PATH = File.join(ROOT, ".csdlc/evidence/161/proof.json")

def fail!(message)
  abort("FAIL [V3-01]: " + message)
end
...
puts "PASS: V3-01 exact-head artifact, producer, and invariant proof"
