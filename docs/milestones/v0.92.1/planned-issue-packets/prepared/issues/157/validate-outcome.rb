...
ISSUE = 157
ID = "corp-05"
EVIDENCE_DIR = File.join(ROOT, "docs/milestones/v0.92.1/evidence/corporate/corp-05")
OPERATIONS_DIR = File.join(ROOT, "docs/operations/corporate/corp-05")
MANIFEST_PATH = File.join(ROOT, ".csdlc/evidence/157/evidence-manifest.json")
REQUIRED_ARTIFACTS = ["administrative-control-manifest.json", "no-touch-readback.json", "reference-dispositions.json", "copy-cutover-runbook.md"].freeze
REQUIRED_RECEIPTS = ["immutable-ref-readback", "visibility-readback", "no-touch-readback", "control-surface-readback"].freeze
...
control = load_json(artifact("administrative-control-manifest.json"), "control manifest")
no_touch = load_json(artifact("no-touch-readback.json"), "no-touch readback")
refs = load_json(artifact("reference-dispositions.json"), "reference dispositions")
repos = array(control, "repositories")
assert(repos.length == 7, "approved repository denominator must be seven")
repos.each do |row|
  %w[name source_owner destination_owner source_ref destination_ref visibility readback].each { |field| present(row, field, "repository row") }
  assert(row["source_ref"] == row["destination_ref"], "repository ref parity failed")
  expected_visibility = row["name"] == "agent-design-language" ? "public" : "private"
  assert(row["visibility"] == expected_visibility, "repository visibility matrix violated")
  assert(row["source_mutated"] == false, "founder source repository was mutated")
end
untouched = array(no_touch, "repositories")
assert(untouched.map { |row| row["name"] }.sort == %w[Horust asksifu].sort, "no-touch denominator mismatch")
assert(untouched.all? { |row| row["unchanged"] == true }, "excluded repository changed")
required_surfaces = %w[domains brands apps webhooks packages pages oidc repository_references]
surface_rows = array(refs, "surfaces")
assert((required_surfaces - surface_rows.map { |row| row["kind"] }).empty?, "control-surface denominator incomplete")
assert(surface_rows.all? { |row| %w[verified retained excluded blocked].include?(row["disposition"]) }, "invalid control-surface disposition")
require_runbook_sections(artifact("copy-cutover-runbook.md"), %w[Allowlist Copy Verification Cutover Rollback No-touch Cleanup])
