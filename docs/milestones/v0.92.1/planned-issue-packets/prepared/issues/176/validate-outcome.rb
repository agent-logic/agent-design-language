...
ISSUE = 176
WORK_PACKAGE = "V3-13"
REQUIRED_ARTIFACTS = ["csdlc-v3/src/adapters/github/read/mod.rs", "csdlc-v3/src/adapters/github/read/issues.rs", "csdlc-v3/src/adapters/github/read/pulls.rs", "csdlc-v3/src/adapters/github/read/checks.rs", "csdlc-v3/src/adapters/github/read/principals.rs", "csdlc-v3/tests/github/read/issues.rs", "csdlc-v3/tests/github/read/checks.rs", "csdlc-v3/tests/github/read/fixtures.rs"].freeze
REQUIRED_LANES = {"v3-13-focused-rust"=>["cargo", "test", "--locked", "--manifest-path", "csdlc-v3/Cargo.toml", "--all-targets"], "v3-13-diff-hygiene"=>["git", "diff", "--check", "origin/main...HEAD"]}.freeze
EXPECTED_OBSERVATIONS = {"domain_octocrab_type_dependency_count"=>["eq", 0], "collapsed_github_error_kind_count"=>["eq", 0], "check_without_exact_head_count"=>["eq", 0], "null_state_normalized_open_count"=>["eq", 0], "http404_normalized_open_count"=>["eq", 0], "undocumented_raw_endpoint_count"=>["eq", 0], "read_command_mutation_count"=>["eq", 0], "credential_url_or_log_leak_count"=>["eq", 0]}.freeze
PROOF_PATH = File.join(ROOT, ".csdlc/evidence/176/proof.json")

def fail!(message)
  abort("FAIL [V3-13]: " + message)
end
...
puts "PASS: V3-13 exact-head artifact, producer, and invariant proof"
