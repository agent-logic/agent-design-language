...
ISSUE = 169
WORK_PACKAGE = "V3-08"
REQUIRED_ARTIFACTS = ["csdlc-v3/src/store/mod.rs", "csdlc-v3/src/store/transaction.rs", "csdlc-v3/src/store/recovery.rs", "csdlc-v3/src/recovery/mod.rs", "csdlc-v3/tests/store/atomic_commit.rs", "csdlc-v3/tests/store/interruption_matrix.rs", "csdlc-v3/tests/store/remote_intents.rs", "csdlc-v3/tests/store/platform_capabilities.rs"].freeze
REQUIRED_LANES = {"v3-08-focused-rust"=>["cargo", "test", "--locked", "--manifest-path", "csdlc-v3/Cargo.toml", "--all-targets"], "v3-08-diff-hygiene"=>["git", "diff", "--check", "origin/main...HEAD"]}.freeze
EXPECTED_OBSERVATIONS = {"authoritative_commit_point_count"=>["eq", 1], "projection_before_state_commit_count"=>["eq", 0], "stale_writer_commit_count"=>["eq", 0], "nonconvergent_interruption_case_count"=>["eq", 0], "remote_operation_before_durable_intent_count"=>["eq", 0], "intent_lifecycle_state_field_count"=>["eq", 0], "unproven_platform_mutation_success_count"=>["eq", 0], "recovery_guess_count"=>["eq", 0]}.freeze
PROOF_PATH = File.join(ROOT, ".csdlc/evidence/169/proof.json")

def fail!(message)
  abort("FAIL [V3-08]: " + message)
end
...
puts "PASS: V3-08 exact-head artifact, producer, and invariant proof"
