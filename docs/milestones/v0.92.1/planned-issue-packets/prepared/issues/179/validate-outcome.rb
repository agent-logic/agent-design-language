...
ISSUE = 179
WORK_PACKAGE = "V3-16"
REQUIRED_ARTIFACTS = ["csdlc-v3/migration/parity-matrix.v1.json", "csdlc-v3/migration/migration-map.v1.json", "csdlc-v3/migration/writer-fence.v1.json", "csdlc-v3/canary/cohort.v1.json", "csdlc-v3/canary/known-defect-regressions.v1.json", "csdlc-v3/install/selector-cutover.json", "csdlc-v3/evidence/cutover/canary-receipts.json", "csdlc-v3/evidence/cutover/authority-audit.json", "csdlc-v3/evidence/cutover/rollback-receipt.json"].freeze
REQUIRED_LANES = {"v3-16-focused-rust"=>["cargo", "test", "--locked", "--manifest-path", "csdlc-v3/Cargo.toml", "--all-targets"], "v3-16-diff-hygiene"=>["git", "diff", "--check", "origin/main...HEAD"]}.freeze
EXPECTED_OBSERVATIONS = {"unexplained_parity_mismatch_count"=>["eq", 0], "unsupported_field_without_disposition_count"=>["eq", 0], "v3_only_canary_failure_count"=>["eq", 0], "card_family_correction_coverage_count"=>["gte", 6], "known_defect_without_regression_or_disposition_count"=>["eq", 0], "dual_writer_window_count"=>["eq", 0], "v2_fence_bypass_count"=>["eq", 0], "unapproved_selector_mutation_count"=>["eq", 0]}.freeze
PROOF_PATH = File.join(ROOT, ".csdlc/evidence/179/proof.json")

def fail!(message)
  abort("FAIL [V3-16]: " + message)
end
...
puts "PASS: V3-16 exact-head artifact, producer, and invariant proof"
