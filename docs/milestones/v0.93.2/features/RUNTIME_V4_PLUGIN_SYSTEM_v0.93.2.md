# v0.93.2 Runtime v4 CSM Plugin System

## Status

The approved #922 split schedules the full Runtime v4 program in v0.93.2. RV-01 through RV-11 are mandatory release scope. The prior design is not automatically approved: RV-01 must resolve its review findings before implementation admission.
...
The local Runtime v4 design and Claude/Gemini reviews were inspected under #1047. Their scope and dispositions are retained in the source inventory and decision register. Tracked foundations are `docs/architecture/RUNTIME_V3_SERVICE_CONTRACT_ARCHITECTURE.md`, `docs/architecture/RUNTIME_V3_OPERATIONAL_COMPONENTS.md` and `adl-runtime-kernel/src/component.rs`. These are pre-extraction paths; v0.93.1/RD-02 maps their destination.
...
RV-01 through RV-03 establish the native generation lifecycle; RV-09 through RV-11 prove state migration, reconfiguration and removal before non-native adapters proceed. RV-04 through RV-08 must then complete process and WASM adapters, cross-adapter equivalence, provider integration and installed recovery qualification in v0.93.2. An intermediate native-only acceptance cannot satisfy the milestone release gate. No cluster-wide activation, automatic registry download, ambient credentials or unreviewed state migration is admitted.
...
Execution owners: agent-logic-runtime. Candidate results: RV-01 through RV-11 in dependency order. Named people and exact repository identifiers are settled before execution. Shared contracts have one producer and versioned consumers; source is not duplicated between owners.
...
- **RV-03:** The durable generation transaction stages, quiesces, activates and rolls back to one fenced generation after interruption; in-flight work and cancellation retain their declared disposition.
- **RV-09:** An installed plugin migrates declared prior-version state to the selected version and preserves the prior generation when the migrator refuses or fails.
- **RV-10:** Versioned configuration migration and two-phase validation/activation apply one accepted configuration, or preserve the prior version and active configuration on refusal, failure or interruption.
- **RV-11:** A requested plugin removal checks dependencies and in-flight work, refuses unsafe removal, and completes authorized removal with durable recovery state.
...
- RV-01 follows WP-01, consuming the accepted v0.93.1/RD-11 lockset.
- RV-02 follows RV-01.
- RV-03 follows RV-02.
- RV-09 follows RV-03.
- RV-10 follows RV-09.
...
- RV-03: Interrupted activation loses owner; Stale generation accepts work; Rollback duplicates in-flight delivery.
- RV-09: Unsupported state version; Migrator failure mutates prior state; Retry repeats non-idempotent migration.
- RV-10: Invalid configuration activated; Interrupted apply loses known-good configuration; Credential scope changes silently; Unsupported configuration version; Configuration migrator failure loses prior version.
- RV-11: Live dependent orphaned; Unsafe removal accepted; Interrupted removal resurrects stale generation.
...
RV-01, RV-02, RV-03, RV-09, RV-10, RV-11, RV-04, RV-05, RV-06, RV-07, RV-08
