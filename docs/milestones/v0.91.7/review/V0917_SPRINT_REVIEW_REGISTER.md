...
| WP-07A CSM runtime rearchitecture and topology sprint | #5121 closed; #5409/#5494 remediation implemented, exact re-review and merge/closeout pending | `docs/reviews/v0.91.7/remaining-sprints-5403/WP07A_REARCHITECTURE_REVIEW_5121.md`; `docs/review-fixes/runtime/WP07A_REARCHITECTURE_REPAIR_5409.md` | The corrective implementation reports the actual daemon-supervised-cycle model, derives readiness from all required CSM component and policy-required typed-channel observations, runs 100 supervised Runtime v3 task cycles through all real typed channels with injected failure/restart/recovery and durable replay, and separately proves the unmodified production daemon entrypoint across real ticks plus failure/recovery. It also provides serialized bounded credential overlap for bearer and matching gateway signatures, expired-generation recovery, monotonic terminal revocation, and authorization/revocation serialization. Runtime v3 retains separate weather ownership. All ten findings have implementation and local validation evidence; exact-head re-review remains pending. The umbrella remains setup/topology evidence only. | Complete exact-head re-review and merge #5494; close #5409 only after lifecycle closeout. |
