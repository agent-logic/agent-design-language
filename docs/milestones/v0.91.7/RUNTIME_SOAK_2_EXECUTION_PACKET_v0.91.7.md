...
| Agent lifecycle | `#4245` long-lived-agent continuity and status snapshots | lifecycle transitions and negative copied/restart cases under the assembled runtime | Soak #2 matrix/setup | block; Soak #3 only with operator approval |
...
| Resilience | `#4245` resilience traces plus late `#4547` handoff when published | retry, timeout, cancellation, degraded mode, and partial-failure proof under the assembled runtime | Soak #2 standard run | block; Soak #3 only with operator approval |
...
| Runtime AWS and signal bridge | `#4545` AWS profile, SSM, and CloudWatch truth | heartbeat publisher, ACIP-to-SNS, and runtime AWS signal path are integrated/proven or explicitly blocked with evidence | parallel AWS/signal lane | block unless operator approves a non-claim |
| Observatory / Unity live consumption | `#4245` prerequisite consumption only; late `#4548` handoff when published | live runtime state is consumed, not just canned demo data | Observatory lane tied to Soak #2 closeout | block; Soak #3 only with operator approval |
...
| Capability envelope | feature-list and handoff docs only | capability/authority limits are explicit for the assembled runtime and temporary agents | Soak #2 matrix/setup | operator-approved non-claim only if not required for the standard run |
...
| Curiosity / Constructability, if promoted before run | v0.91.7 bridge docs only | one governed discovery/admissibility cycle if those surfaces land in time; otherwise explicit operator-approved non-claim | explicit optional Soak #2 row | non-claim or blocker with evidence |
