...
Each real issue must carry one completed result, repository owner, exact dependencies, consumer acceptance, negative/recovery cases, resource envelope, non-goals and native lifecycle cards. If implementation discovery requires splitting a candidate, preserve its full parent acceptance and all dependency edges; do not close the original after a scaffold-only child.
