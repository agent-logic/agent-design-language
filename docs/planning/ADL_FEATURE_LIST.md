...
| `subsystem_complete` | The feature has explicit done criteria and all required subsystem behavior is implemented or explicitly out of scope. | Done criteria, owner issue/PR evidence, validation, review, and evidence-backed blocker disposition. |
...
proof surface, and evidence-backed blocker or scope disposition. A baseline row without those four
...
Active milestone: v0.92.2; v0.93 is forward planning, not open.
...
- `v0.92.1` supplies retained predecessor evidence; v0.92.2 owns the current
  CodeFriend Beta 1 release tail, and v0.93 owns the selected successor scope
...
| Logging, observability, and OTel-compatible proof-loop readiness | Retained bounded prerequisite and consumer proof | `docs/milestones/v0.91.5/CONTROL_PLANE_OBSERVABILITY_CONTRACT_3609.md`, `docs/milestones/v0.91.5/SHARED_OBSERVABILITY_AND_OTEL_CONTRACT_3705.md`, `docs/milestones/v0.91.5/OPEN_TELEMETRY_INTEGRATION_BOUNDARY_3709.md`, closed `#4718`, retained Soak #2, and WP-09/WP-15 consumption packets | `#4718` is the retained prerequisite proof for parse-safe JSON, stderr `adl_event` behavior, redaction hygiene, and OTel-compatible mapping. Soak and Observatory consumers provide bounded consumption evidence; Unity live production export remains unclaimed. |
| Resilience, citizen persistence, and operational sleep/wake | Retained bounded integration proof | `docs/milestones/v0.91.5/features/AEE_COMPLETION_TRANCHE_v0.91.5.md`, `docs/milestones/v0.91.6/features/RESILIENCE_PERSISTENCE_SLEEP_WAKE_v0.91.6.md`, closed `#4778`, closed `#4780`-`#4784`, and `docs/milestones/v0.91.7/review/V0917_RESILIENCE_INTEGRATION_MINI_SPRINT_4778.md` | v0.92 may consume only the retained workflow, provider/model, remote, runtime-middleware, and failure-injection proof. Full product resilience beyond those surfaces is not claimed. |
| Public prompt records export, redaction, validation, and indexing | Required pre-v0.92 implementation/proof surface | local prompt-card authoring surface, public prompt-record export work, `docs/milestones/v0.91.5/features/PUBLIC_PROMPT_RECORDS_v0.91.5.md`, and `docs/milestones/v0.91.6/features/PUBLIC_PROMPT_RECORDS_EXPORT_v0.91.6.md` | Local cards remain editable; v0.92 may consume public records only after export, redaction, validation, indexing, security, and evidence boundaries are implemented or blocked with evidence. |
| Provider/model reliability and multi-agent readiness | Retained bounded implementation/proof | Sprint 2/remediation proof packets, closed WP-05, provider mini-sprint packets, and scheduler/provider/local-agent review evidence | Reliability proof remains separate from training or ANRM product claims; v0.92 may consume only paths with role suitability, known failure modes, and retained proof. |
...
| Opening repository split | Planned; operator-selected v0.93 opening phase | `docs/milestones/v0.93/REPOSITORY_MIGRATION_v0.93.md` and `EXECUTION_PLAN_v0.93.json` | RD-01 through RD-13 after accepted v0.92.2 closure and explicit opening; one public ADL plus six private repositories; CodeFriend software separate from existing website; complete before new feature work |
| Runtime v4 CSM plugin system | Required v0.93 outcome; not implemented by planning | `docs/milestones/v0.93/features/RUNTIME_V4_PLUGIN_SYSTEM_v0.93.md` | RV-01 through RV-11: native/process/WASM adapters, migration/reconfiguration/removal, fenced recovery, provider integration and installed qualification; cannot be deferred as a reduced native-only release |
...
| CodeFriend versioned artifact templates and style guides | Required v0.93; operator confirmed | `docs/milestones/v0.93/features/CODEFRIEND_ARTIFACT_TEMPLATES_AND_STYLE_GUIDES_v0.93.md` | CT-01–CT-10; fill-in-the-blank preparation, REV 01 branded output, all supported documents/diagrams/reviews/tests and full 4+1; qualify before CF-05 |
| Citizen reproduction and migration | Required v0.93; operator confirmed | `docs/milestones/v0.93/features/CITIZEN_REPRODUCTION_AND_MIGRATION_v0.93.md` | CM-01–CM-04; governed same-identity migration, new-identity descendants, lineage, scoped inheritance and recovery before INTEGRATE |
...
### v0.91.5 - Provider, Prompt-Record, And Tooling Pressure

`v0.91.5` carries pressure between the C-SDLC default-operation band and
...
- tracked issue ownership for work that must not be hidden inside v0.92 WP-01

### v0.91.6 - First Required Pre-v0.92 Readiness Tranche

`v0.91.6` finished the first load-bearing readiness work:
...
- security readiness and Continuous Adversarial Verification
- ACIP/A2A/protobuf/JSON/WebSocket projection decisions
- logging/observability/OTel consumption from closed `#4718`
- resilience integration from `#4778` and `#4780`-`#4783`

This is not a general spillover milestone. It exists so v0.92 consumes
implemented and integrated proof, or an explicit evidence-backed blocker with
operator approval, rather than rediscovering unresolved scope during WP-01.
...
implementation path or the v0.92 birthday milestone.
