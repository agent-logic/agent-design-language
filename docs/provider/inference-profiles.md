...
- `temperature`, `top_p`, `max_output_tokens`, and `timeout_secs` are present
  after expansion and validated before activation.
...
- `profile_state.schema: adl.provider_profile_state.v1` retains the
  `last_known_good_profile` from the previous valid state when one is supplied
  and requires `validate_before_activation`.
...
- Deterministic byte evidence uses
  ...
  provider/config projection. Raw `AdlDoc` provider maps keep their public
