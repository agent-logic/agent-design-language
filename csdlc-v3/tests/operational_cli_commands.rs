...
fn bind_recovers_after_process_exit_following_git_side_effect() {
    ...
    assert!(fixture.root.join(".git/csdlc-v3/local/transactions/505.json").is_file());
    ...
    assert!(!fixture.root.join(".git/csdlc-v3/local/transactions/505.json").exists());
}
...
fn edit_recovers_after_process_exit_between_directory_swaps() {
    ...
    let values: serde_json::Value = serde_json::from_slice(
        &fs::read(fixture.root.join(".git/csdlc-v3/local/issues/505/cards/sip.values.json")).unwrap(),
    )
    .unwrap();
