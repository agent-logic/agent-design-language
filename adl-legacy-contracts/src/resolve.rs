...
pub fn resolve_run(doc: &adl::AdlDoc) -> Result<AdlResolved> {
    ...
    if doc
        .providers
        .values()
        .any(|p| p.profile.is_some() && p.kind.trim().is_empty())
    {
        return Err(anyhow!(
            "provider profiles require Runtime expansion before portable resolution"
        ));
    }
    let _version = parse_version(&doc.version)?;

    let run_id = crate::validate_run_id_path_segment(doc.run.name.as_deref().unwrap_or("run"))
...
mod tests {
    ...
    // PVF: deterministic contract regression, CPU-only, required RD03 gate.
    #[test]
    fn portable_resolution_accepts_materialized_profile_but_rejects_unexpanded_profile() {
        let mut doc = minimal_doc();
        let provider = doc.providers.get_mut("local").unwrap();
        provider.profile = Some("runtime-expanded-profile".into());
        assert!(resolve_run(&doc).is_ok());
        doc.providers.get_mut("local").unwrap().kind.clear();
        let error = resolve_run(&doc).unwrap_err().to_string();
        assert!(error.contains("Runtime expansion"));
    }
