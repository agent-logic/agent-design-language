...
/// Package version, independent of the UTS wire schema version.
pub const PACKAGE_VERSION: &str = env!("CARGO_PKG_VERSION");
pub const V1_SCHEMA_JSON: &str =
    include_str!("../schemas/v1.0/universal_tool_schema.v1.schema.json");
pub const V1_1_SCHEMA_JSON: &str =
    include_str!("../schemas/v1.1/universal_tool_schema.v1_1.schema.json");
pub const V1_1_INVOCATION_SCHEMA_JSON: &str =
    include_str!("../schemas/v1.1/tool_invocation.v1_1.schema.json");

/// Load the two supported declaration versions. Validate before upgrading v1:
/// normalization must never erase an unsupported version or invalid declaration.
/// This performs schema/semantic validation only, never execution authorization.
pub fn load_tool_declaration(value: JsonValue) -> Result<UniversalToolSchemaV1_1, String> {
    match value.get("schema_version").and_then(JsonValue::as_str) {
        Some(UTS_SCHEMA_VERSION_V1_0) => {
            let schema: UniversalToolSchemaV1 = serde_json::from_value(value)
                .map_err(|_| "malformed_tool_declaration".to_string())?;
            validate_uts_v1(&schema).map_err(|report| {
                format!("invalid_tool_declaration:{}", report.codes().join(","))
            })?;
            Ok(upgrade_uts_v1_to_v1_1(schema))
        }
        Some(UTS_SCHEMA_VERSION_V1_1) => {
            let schema: UniversalToolSchemaV1_1 = serde_json::from_value(value)
                .map_err(|_| "malformed_tool_declaration".to_string())?;
            validate_uts_v1_1(&schema).map_err(|report| {
                format!("invalid_tool_declaration:{}", report.codes().join(","))
            })?;
            Ok(schema)
        }
        _ => Err("unsupported_schema_version".to_string()),
    }
}
...
mod tests {
    ...
    // PVF: runtime; contract compatibility; deterministic CPU-only; required #877.
    #[test]
    fn packaged_loader_accepts_supported_versions_without_semantic_drift() {
        let v1 = parse_valid(valid_safe_read_json());
        let loaded = load_tool_declaration(serde_json::to_value(&v1).unwrap()).unwrap();
        assert_eq!(loaded, upgrade_uts_v1_to_v1_1(v1));
        let v11 = parse_valid_v1_1(valid_safe_read_v1_1_json());
        assert_eq!(
            load_tool_declaration(serde_json::to_value(&v11).unwrap()).unwrap(),
            v11
        );
        ...
        for schema in [
            V1_SCHEMA_JSON,
            V1_1_SCHEMA_JSON,
            V1_1_INVOCATION_SCHEMA_JSON,
        ] {
            let value: JsonValue = serde_json::from_str(schema).unwrap();
            assert!(value["$id"].as_str().unwrap().contains("uts"));
        }
    }

    // PVF: runtime; reject invalid contracts before consumer dispatch; local deterministic.
    #[test]
    fn packaged_loader_rejects_unknown_versions_and_malformed_declarations() {
        for version in ["uts.v0", "uts.v2", "0.1.0", "", "uts.v1.2"] {
            let mut value = valid_safe_read_json();
            value["schema_version"] = json!(version);
            assert_eq!(
                load_tool_declaration(value).unwrap_err(),
                "unsupported_schema_version"
            );
        }
        let mut value = valid_safe_read_json();
        ...
        assert_eq!(
            load_tool_declaration(value).unwrap_err(),
            "malformed_tool_declaration"
        );
        let mut value = valid_safe_read_json();
        value["name"] = json!("!");
        assert!(load_tool_declaration(value)
            .unwrap_err()
