...
ISSUE = 5874
MARKER = "ADL_ISSUE_5874_NEGATIVE_CASE_V1 "
EXPECTED = %w[
...
NEGATIVE = ["cargo", "test", "--manifest-path", "adl-runtime/Cargo.toml", "--test", "distributed_snapshot_catalog", "--", "--nocapture"].freeze
ROOT = Pathname.new(__dir__).join("../../..").cleanpath.expand_path
EVIDENCE = ROOT.join(".csdlc/evidence/5874")
...
runner = {"provider" => "local-codex", "run_id" => "5874-local-operator-v1", "os" => "macos", "arch" => "aarch64", "identity_sha256" => sha(runner_path)}
...
proof = {
  "schema" => "adl.wp04.execution_proof.v3", "issue" => ISSUE, "wp" => "WP-04.12", "source_revision" => source,
  "evidence_revision_strategy" => "derive_from_receipt_introduction",
  "protected_paths" => ["adl-runtime/src/distributed/snapshot_catalog.rs", "adl-runtime/tests/distributed_snapshot_catalog.rs"],
  "source_artifacts" => ["adl-runtime/src/distributed/snapshot_catalog.rs", "adl-runtime/tests/distributed_snapshot_catalog.rs"].map { |path| {"path" => path, "sha256" => Digest::SHA256.hexdigest(`git -C #{ROOT} show #{source}:#{path}`)} },
