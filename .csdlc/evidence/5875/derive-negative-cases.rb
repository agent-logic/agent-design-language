...
ISSUE = 5875
MARKER = "ADL_ISSUE_5875_NEGATIVE_CASE_V1 "
EXPECTED = %w[
  unknown_migration rollback_restart corrupt_restart zero_capacity skip_after_abort
  post_fence_abort wrong_activation_key
].to_h { |name| [name, "rejected"] }.freeze
EXACT = ["ruby", ".csdlc/evidence/5875/run-exact-child-tests.rb", "cargo", "nextest", "run", "--manifest-path", "adl-runtime/Cargo.toml", "--test", "distributed_migration", "--no-tests=fail"].freeze
CLIPPY = ["cargo", "clippy", "--manifest-path", "adl-runtime/Cargo.toml", "--test", "distributed_migration", "--", "-D", "warnings"].freeze
NEGATIVE = ["cargo", "test", "--manifest-path", "adl-runtime/Cargo.toml", "--test", "distributed_migration", "--", "--nocapture"].freeze
ROOT = Pathname.new(__dir__).join("../../..").cleanpath.expand_path
EVIDENCE = ROOT.join(".csdlc/evidence/5875")
...
runner = {"provider" => "local-codex", "run_id" => "5875-local-operator-v1", "os" => "macos", "arch" => "aarch64", "identity_sha256" => sha(runner_path)}
...
protected_paths = ["adl-runtime/src/distributed/migration.rs", "adl-runtime/tests/distributed_migration.rs"]
proof = {
  "schema" => "adl.wp04.execution_proof.v3", "issue" => ISSUE, "wp" => "WP-04.13", "source_revision" => source,
