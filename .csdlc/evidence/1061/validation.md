...
- `cargo test --manifest-path csdlc-v3/Cargo.toml --lib commands::remote::coordination`: 9 passed, 0 failed.
- `cargo test --manifest-path csdlc-v3/Cargo.toml --test installed_coordination_completion`: 14 passed, 0 failed.
...
The installed suite covers atomic marker installation and completion, exact replay, stale body and timestamp, malformed or pre-existing markers, wrong repository/issue/children, missing approval, changed replay, semantic-state rejection, authenticated child/readback gates, and continued denial of ordinary legacy mutations.
