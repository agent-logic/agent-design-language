...
NEGATIVE_COMMAND = [
  ...
  "distributed_lease", "quorum_fence_and_restart_safety_machine_evidence", "--", "--exact", "--nocapture"
].freeze
...
def observed_cases(stdout)
  ...
  abort_with("negative-case denominator or names mismatch") unless
    names.length == EXPECTED.length && names.uniq.length == names.length && names.sort == EXPECTED.keys.sort
  entries.each do |entry|
    abort_with("negative-case result mismatch: #{entry.fetch('case')}") unless
      EXPECTED.fetch(entry.fetch("case")) == entry.fetch("result")
  end
...
abort_with("exact nextest denominator mismatch") unless
  summary && summary[1].to_i.positive? && summary[1] == summary[2]
