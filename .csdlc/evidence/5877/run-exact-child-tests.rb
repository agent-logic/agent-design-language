...
require "open3"
require "pathname"
EXPECTED = ["cargo", "nextest", "run", "--manifest-path", "adl-runtime/Cargo.toml", "--test", "distributed_projection", "--no-tests=fail"].freeze
...
$stderr.write(stdout); $stderr.write(stderr)
abort "exact child tests failed" unless status.success?
summary = (stdout + stderr).match(/Summary .*?(\d+) tests run: (\d+) passed, 0 skipped/)
abort "exact child denominator mismatch" unless summary && summary[1].to_i.positive? && summary[1] == summary[2]
