...
The production owner is 6,586 lines across cohesive modules after integrating the independently merged #1006 coordination-completion owner. The increase over the baseline comes from explicit module boundaries, imports, module documentation, and that new bounded responsibility. The two retained test files remain byte-for-byte unchanged.

| Module | Lines | Responsibility |
| --- | ---: | --- |
| `mod.rs` | 49 | Stable public re-exports and test-only compatibility aliases |
| `model.rs` | 567 | Public request, receipt, result, merge, coordination, and dispatch contracts |
...
| `merge_linkage.rs` | 242 | Qualified issue linkage and merge observation query |
| `transport.rs` | 897 | Credential-scoped GitHub transport, response validation, and reconciliation |
| `publication.rs` | 867 | Review, publication, and authenticated PR-readback admission |
| `coordination.rs` | 300 | Coordination-only completion admission, authenticated readiness, and evidence binding |
| `merge.rs` | 828 | Merge eligibility, durable intent, dispatch, and replay |
| `mutation.rs` | 894 | Typed mutation staging, admission, and orchestration |
| `routing.rs` | 91 | Operational route dispatch after authority admission |
| `intent.rs` | 904 | Retained intent observation, recovery, and high-level orchestration |
...
| Coordination-only completion contract, readiness rechecks, and evidence binding | `coordination.rs` |
...
| `cargo test --manifest-path csdlc-v3/Cargo.toml --lib commands::remote` | 72 passed | unit and controlled fake remote transport, including coordination completion; deterministic; bounded CPU/disk; required gate |
| `cargo test --manifest-path csdlc-v3/Cargo.toml --test installed_coordination_completion` | 4 passed | installed coordination lifecycle integration; deterministic fixtures and bounded local IO; required compatibility gate |
