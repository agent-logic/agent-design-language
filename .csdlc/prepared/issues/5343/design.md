...
by #5344. Runtime v2 and incumbent ADL are untouched rollback targets and may
not be edited, copied, imported, linked, or deleted.
...
5. Re-read the selector and verify the exact transaction receipt and selected
   executable identity.
6. Run the bounded fresh-install smoke and explicit v1 override proof without
   changing or deleting the v1 installation.
...
- #5344 lacks live merge, typed closeout, claim release, retained receipt,
  ancestry, or accepted exact evidence;
- the #5345 selector/installer is not merged, typed closed out, receipt-backed,
  ancestral, or exact-install verifiable;
