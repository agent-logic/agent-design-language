...
ISSUE = 173
WORK_PACKAGE = "V3-11A"
REQUIRED_ARTIFACTS = ["csdlc-v3/src/pvf/plan/mod.rs", "csdlc-v3/src/pvf/plan/manifest.rs", "csdlc-v3/src/pvf/plan/dag.rs", "csdlc-v3/tests/pvf/plan/classification.rs", "csdlc-v3/tests/pvf/plan/acceptance_coverage.rs", "csdlc-v3/tests/pvf/plan/dag.rs"].freeze
REQUIRED_LANES = {"v3-11a-focused-rust"=>["cargo", "test", "--locked", "--manifest-path", "csdlc-v3/Cargo.toml", "--all-targets"], "v3-11a-diff-hygiene"=>["git", "diff", "--check", "origin/main...HEAD"]}.freeze
EXPECTED_OBSERVATIONS = {"unclassified_lane_state_count"=>["eq", 0], "missing_acceptance_coverage_count"=>["eq", 0], "duplicate_acceptance_owner_count"=>["eq", 0], "accepted_cycle_count"=>["eq", 0], "hidden_routing_policy_count"=>["eq", 0], "ambient_effect_call_count"=>["eq", 0]}.freeze
PROOF_PATH = File.join(ROOT, ".csdlc/evidence/173/proof.json")

def fail!(message)
  abort("FAIL [V3-11A]: " + message)
end
...
puts "PASS: V3-11A exact-head artifact, producer, and invariant proof"
