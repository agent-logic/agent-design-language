...
| #268 | Retained Runtime EBS volume attachment | `retained_runtime_volume`, `retained_volume_id`, `runtime_mount_path` | AWS `aws_volume_attachment.retained_runtime`; GCP requires a retained persistent disk, attaches it with a stable device name, mounts it at `/opt/adl-runtime`, and verifies `/opt/adl-runtime/runtime/install`. |
...
| #268 | Amazon SSM managed instance role and pinned S3 object-version read | `service_identity`, `artifact_source` | AWS IAM role/profile with SSM managed policy and `s3:GetObjectVersion`; GCP service account plus optional `roles/storage.objectViewer` bucket binding. |
...
| #268 | Bootstrap log, readiness marker, and readiness command | `bootstrap_log_path`, `readiness_marker_path`, `readiness_command` | AWS user data writes `/var/log/adl-issue268-bootstrap.log` and `/var/lib/adl/issue268-bootstrap-ready`; GCP startup script writes the same readiness marker only after retained disk mount and Runtime install proof. |
