...
def errors_for(report, expected_paths)
  ...
  errors << "review status invalid" unless %w[final superseded].include?(report["status"])
  ...
  errors << "validator evidence absent" if validators.empty? || validators.any? { |row| !%w[passed failed].include?(row["status"]) || !row["argv"].is_a?(Array) || row["argv"].empty? || !row["evidence_sha256"].to_s.match?(/\A[0-9a-f]{64}\z/) }
...
if ARGV == ["--negative"]
  ...
  base = {"reviewed_revision" => revision, "predecessor" => {"issue" => 524, "reviewed" => true, "merged" => true, "merge_sha" => "b" * 40},
          ...
          "validator_results" => [{"argv" => ["ruby", "validator.rb"], "status" => "passed", "evidence_sha256" => "d" * 64}],
...
revision = report["reviewed_revision"]
