...
if failures.empty?
  puts JSON.generate(status: "ready", issues: ISSUES, conductor_gate: 5499, interface_gate: 5349)
  exit 0
end

puts JSON.generate(status: "waiting", issues: ISSUES, conductor_gate: 5499, interface_gate: 5349, blockers: failures)
exit 3
