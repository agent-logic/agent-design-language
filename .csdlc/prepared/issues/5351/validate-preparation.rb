...
assert(claim.fetch("id") == "claim-5351-v0918-wp16-quality-gate-preparation", "wrong claim")
...
assert(index.fetch("design_path") == ".csdlc/prepared/issues/5351/design.md", "wrong design path")
assert(index.fetch("diagram_path") == ".csdlc/prepared/issues/5351/diagram.mmd", "wrong diagram path")
...
REQUIRED_CARDS.each do |name|
  ...
  assert(identity.fetch("title").include?("WP-16"), "#{name} title is not issue-specific")
  assert(identity.fetch("slug") == "v0918-wp16-integrated-platform-quality-gate", "#{name} slug mismatch")
  ...
  rendered_identity = rendered.lines.map do |line|
    match = line.match(/\A(Template|Issue|Repository|Card): ([^\r\n]+)\r?\n?\z/)
    [match[1], match[2]] if match
  end.compact.to_h
  assert(rendered_identity.fetch("Template") == native.fetch("template_set"), "#{name} rendered template mismatch")
  assert(rendered_identity.fetch("Issue") == ISSUE.to_s, "#{name} rendered issue mismatch")
  assert(rendered_identity.fetch("Repository") == "danielbaustin/agent-design-language", "#{name} rendered repository mismatch")
  assert(rendered_identity.fetch("Card") == name, "#{name} rendered kind mismatch")
end

vpp = JSON.parse(ISSUE_DIR.join("cards/vpp.values.json").read).dig("content", "values")
expected_lanes = %w[preparation-contract wp15-terminal-gate focused-quality integrated-platform complete post-merge-exact]
...
expected_lane_contracts = {
  "preparation-contract" => ["small", 120, ["AC-3", "AC-6", "AC-7"]],
  "wp15-terminal-gate" => ["small", 120, ["AC-1"]],
  "focused-quality" => ["medium", 600, ["AC-2", "AC-3", "AC-5", "AC-6", "AC-7"]],
  "integrated-platform" => ["large", 1800, ["AC-2", "AC-3", "AC-4", "AC-5", "AC-6", "AC-7"]],
  "complete" => ["large", 2280, %w[AC-1 AC-2 AC-3 AC-4 AC-5 AC-6 AC-7 AC-8]],
  "post-merge-exact" => ["large", 2280, %w[AC-1 AC-2 AC-3 AC-4 AC-5 AC-6 AC-7 AC-8]]
}.freeze
lanes.each do |entry|
...
puts JSON.generate(status: "pass", issue: ISSUE, phase: index.fetch("phase"), cards: 6, product_changes: 0, typed_doctor: "pass", lines: line_counts)
