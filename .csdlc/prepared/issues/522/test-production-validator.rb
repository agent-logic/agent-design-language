...
Dir.mktmpdir("issue-522-production-",File.expand_path("../../../../.adl",__dir__)) do |repo|
 Dir.chdir(repo) do
  ...
  sh!("git","add","."); sh!("git","commit","-qm","remediation"); head=`git rev-parse HEAD`.strip; File.write("ledger","candidate"); sh!("git","add","."); sh!("git","commit","-qm","ledger"); ledger=`git rev-parse HEAD`.strip
  ...
  authority_path=File.join(root,"authority.json"); wj(authority_path,{"outcome"=>"passed","reviewed_sha"=>head,"findings"=>[],"blockers"=>[],"authority_kind"=>"typed_csdlc"})
  validation_path=File.join(root,"validation.json"); wj(validation_path,{"outcome"=>"passed","head_sha"=>head,"failures"=>[]})
  stdout=JSON.generate({"outcome"=>"passed","head_sha"=>head,"failures"=>[]})+"\n"; invocation_path=File.join(root,"invocation.json"); validator_blob=`git show #{head}:#{validator_path}`; wj(invocation_path,{"validator_path"=>validator_path,"validator_sha256"=>Digest::SHA256.hexdigest(validator_blob),"argv"=>["ruby",validator_path],"arguments"=>[],"exit_status"=>0,"head_sha"=>head,"stdout"=>stdout,"stdout_sha256"=>Digest::SHA256.hexdigest(stdout)})
  source_path=File.join(root,"source-findings.json"); wj(source_path,{"ledger_candidate_sha"=>ledger,"reports"=>reports,"findings"=>[finding],"zero_findings_proof"=>"n/a"})
  remediation={"issue"=>700,"pull_request"=>902,"head_sha"=>head,"merge_sha"=>head,"artifacts"=>[{"path"=>"remediation/fix.txt","sha256"=>Digest::SHA256.hexdigest(`git show #{head}:remediation/fix.txt`)}]}
  review={"authority_kind"=>"typed_csdlc","assignment_id"=>"a1","reviewer"=>"reviewer","authority_receipt_path"=>authority_path,"authority_receipt_sha256"=>Digest::SHA256.file(authority_path).hexdigest,"head_sha"=>head,"reviewed_sha"=>head,"observed_pr_head_sha"=>head,"observed_at"=>"now","report_path"=>review_path,"sha256"=>Digest::SHA256.file(review_path).hexdigest}
  ...
  paths=[source_path,disposition_path,blockers_path,review_path,authority_path,validation_path,invocation_path]; manifest_path=File.join(root,"packet-manifest.json"); wj(manifest_path,{"entries"=>paths.map{|p|{"path"=>p,"sha256"=>Digest::SHA256.file(p).hexdigest}}})
  ...
  reject.call("fabricated_review_receipt",[authority_path,manifest_path]){d=JSON.parse(File.read(authority_path));d["reviewed_sha"]="0"*40;wj(authority_path,d)}
