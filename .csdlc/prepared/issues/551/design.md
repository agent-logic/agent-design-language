...
- Durable continuity identity is read-only. `polis_id` is configuration-owned but restart-gated and never derived from `display_name`.
...
The #510 reload applier atomically replaces only `display_name` when all other identity fields and public endpoints equal the active snapshot. A change to `id`, `public_domain`, `observatory_public_origin`, or Runtime API base is rejected as restart-gated. Any rejected parse or apply retains the complete last-known-good snapshot and emits one bounded, redacted diagnostic that distinguishes parse-invalid from restart-required without echoing configuration values.
...
1. Exact nextest integration targets run with `--no-tests=fail`; configuration tests prove valid identity, missing/duplicate/malformed fields, endpoint mismatch, bounded reload diagnostics, and restart-gated last-known-good retention.
...
- A domain or endpoint change becomes hot-reloadable without TLS/routing authority.
