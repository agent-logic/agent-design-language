...
def proof_manifest(root)
  denominator = JSON.parse(root.join(PROOF_DENOMINATOR).read)
  paths = denominator.fetch("paths")
  fail!("proof denominator count mismatch") unless denominator["expected_path_count"] == 8 && paths.length == 8 && paths.uniq.length == 8
  paths.map do |relative|
    path = root.join(relative)
    fail!("proof path missing: #{relative}") unless path.file?
    { "path" => relative, "sha256" => Digest::SHA256.file(path).hexdigest }
  end
end

def validate_source_denominator(root)
  denominator = JSON.parse(root.join(SOURCE_DENOMINATOR).read)
  fail!("source denominator mismatch") unless denominator["expected_path_count"] == 17 && denominator["paths"] == SOURCE_PATHS
end
...
if options[:self_test]
  repo_text, repo_status = Open3.capture2("git", "rev-parse", "--show-toplevel")
  fail!("cannot resolve repository root") unless repo_status.success?
  repo = Pathname.new(repo_text.strip).realpath
  validate_source_denominator(repo)
  fail!("proof manifest count mismatch") unless proof_manifest(repo).length == 8
  ...
  semantic = root.join(".csdlc/evidence/217/native-platform/linux-semantic.json")
...
validate_source_denominator(root)
...
payload = {
  ...
  "source_denominator_path" => SOURCE_DENOMINATOR, "source_denominator_sha256" => Digest::SHA256.file(root.join(SOURCE_DENOMINATOR)).hexdigest,
  "proof_denominator_path" => PROOF_DENOMINATOR, "proof_denominator_sha256" => Digest::SHA256.file(root.join(PROOF_DENOMINATOR)).hexdigest,
  "proof_manifest" => proof_manifest(root),
