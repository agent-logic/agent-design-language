...
reviewed_children = [5863, 5864, 5865, 5867, 5869, 5870, 5875, 5876]
...
required_architecture = {
  ...
  "## Discovery, Join, And Membership" => ["seeds as addresses, never trust anchors", "deterministic order", "committed epoch", "same effective Guardian control public key"],
  "## Epochs, Leases, And Fencing" => ["openraft", "at least three voters", "non-voting learners", "joint membership", "majority of both", "union majority", "linearization point", "leader term", "committed log index", "certificate generation", "AuthorityCertificateBodyV1", "AuthorityEndorsementV1", "AuthorityEndorsementPayloadV1", "no endorsement metadata is unsigned", "distinct effective control keys", "operation_class", "ed25519", "verify_strict", "adl-authority-endorsement-v1", "unsigned lexicographic", "32-byte", "64-byte", "unknown fields", "duplicate", "Quorum loss", "numerically highest local epoch"],
...
required_threats = {
  ...
  "### T3: Partition-induced split brain" => ["OpenRaft", "majority of both", "union majority", "joint membership", "signer identity", "duplicate effective control public keys", "quorum or clock uncertainty halts mutation"],
...
abort "retained reviewer identity missing" if packet["reviewer"].to_s.strip.empty?
...
abort "retained review agent missing" if provenance["agent_id"].to_s.strip.empty?
