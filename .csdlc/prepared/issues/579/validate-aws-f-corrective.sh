...
reject_runtime_public_edge_resources() {
  if grep -R -E 'resource[[:space:]]+"aws_route53_|resource[[:space:]]+"aws_acm_certificate"' \
    "$ROOT/infra/aws/runtime" "$ROOT/infra/aws/modules/csm-runtime-alb" >/dev/null 2>&1; then
...
reject_world_open_ingress() {
  if awk '
    ...
    END { exit found ? 1 : 0 }
  ' "$ROOT"/infra/aws/modules/csm-runtime-alb/*.tf "$ROOT"/infra/aws/runtime/**/*.tf >/dev/null; then
    :
  else
    echo "AWS-F committed ingress rules must not allow 0.0.0.0/0" >&2
    exit 1
  fi
  if grep -R -F 'allowed_ingress_cidrs = ["0.0.0.0/0"]' "$ROOT/infra/aws/runtime" "$ROOT/infra/aws/modules/csm-runtime-alb" >/dev/null 2>&1; then
...
validate_security_validator_regression() {
  ...
  reject_runtime_public_edge_resources
  reject_world_open_ingress
  reject_text ".csdlc/prepared/issues/579/validate-aws-f-corrective.sh" "0\\\\.0\\\\.0\\\\.0/0"
  reject_text ".csdlc/prepared/issues/579/validate-aws-f-corrective.sh" "grep -v \"egress\""
}
