...
EXPECTED_PROTECTED = [
  ...
  "adl-runtime/src/distributed/polis_runtime.rs", "adl-runtime/tests/distributed_authority_protocol.rs",
...
unless system("git", "merge-base", "--is-ancestor", source, introduction, chdir: ROOT.to_s, out: File::NULL, err: File::NULL)
  introduced = git("diff-tree", "--no-commit-id", "--name-only", "-r", "#{introduction}^", introduction).lines.map(&:strip)
