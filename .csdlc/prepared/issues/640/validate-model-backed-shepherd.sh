...
if [[ "${1:-}" == "--live-wuji" || "${1:-}" == "--capture-live-wuji-before" ]]; then
  ...
  source_guardian="$repo_root/adl-runtime/target/release/adl-runtime-guardian"
  ...
  proof_scope=(
    ...
    adl-runtime-kernel/src adl-runtime-kernel/tests adl-runtime-kernel/Cargo.toml adl-runtime-kernel/Cargo.lock
    adl-runtime/src/guardian.rs adl-runtime/src/bin/adl-runtime-guardian.rs adl-runtime/Cargo.toml adl-runtime/Cargo.lock
  )
  if ! git diff --quiet HEAD -- "${proof_scope[@]}" \
    || [[ -n "$(git ls-files --others --exclude-standard -- "${proof_scope[@]}")" ]]; then
    echo "live Wuji proof requires a clean commit-bound Runtime and Guardian source scope" >&2
    exit 1
  fi
  cargo build --locked --release --manifest-path adl-runtime-kernel/Cargo.toml --bin adl-runtime-kernel
  cargo build --locked --release --manifest-path adl-runtime/Cargo.toml --bin adl-runtime-guardian
  source_kernel_sha256="$(shasum -a 256 "$source_kernel" | awk '{print $1}')"
  source_guardian_sha256="$(shasum -a 256 "$source_guardian" | awk '{print $1}')"
  installed_kernel_sha256="$(shasum -a 256 "$installed_kernel_realpath" | awk '{print $1}')"
  installed_guardian_sha256="$(shasum -a 256 "$installed_guardian_realpath" | awk '{print $1}')"
  ...
  if [[ "$source_guardian_sha256" != "$installed_guardian_sha256" ]]; then
    echo "installed Guardian does not match the exact checked-out source build" >&2
    exit 1
  fi
  ...
  process_guardian_sha256="$(shasum -a 256 "$process_guardian_realpath" | awk '{print $1}')"
  if [[ "$process_guardian_realpath" != "$installed_guardian_realpath" || "$process_guardian_sha256" != "$installed_guardian_sha256" ]]; then
    echo "running Guardian process is not executing the configured exact-build Guardian" >&2
    exit 1
  fi
  public_base_url="$(python3 -c 'import sys,tomllib; print(tomllib.load(open(sys.argv[1], "rb"))["api"]["public_base_url"])' "$init")"
  public_observatory_http_status=""
  for _ in $(seq 1 180); do
    if public_observatory_http_status="$(curl -fsS -o "$feed" -w '%{http_code}' "$public_base_url:20997/v1/observatory")" \
      && jq -e '.agents.sample[] | select(.id == "shepherd" and .state == "ready" and .communication_eligible == true)' "$feed" >/dev/null; then
      break
    fi
    sleep 5
  done
  public_readiness_http_status="$(curl -fsS -o "$ready" -w '%{http_code}' "$public_base_url:20997/v1/ready")"
  python3 - "$status" "$feed" "$ready" "$source_kernel_sha256" "$installed_kernel" "$installed_kernel_realpath" "$process_kernel_realpath" "$before_guardian_pid" "$deployed_kernel_count" "$installed_guardian_realpath" "$process_guardian_realpath" "$source_guardian_sha256" "$public_observatory_http_status" "$public_readiness_http_status" <<'PY'
...
installed_guardian_sha256 = sys.argv[12]
public_observatory_http_status = int(sys.argv[13])
public_readiness_http_status = int(sys.argv[14])
...
assert public_observatory_http_status == 200 and public_readiness_http_status == 200
...
receipt = {
  ...
  "restart_mechanism":"externally_controlled_guardian_restart",
  "guardian_process_before":before_guardian_pid,
  "guardian_process_after":status["guardian_process_id"],
  ...
  "installed_guardian_sha256":installed_guardian_sha256,
  "process_guardian_sha256":installed_guardian_sha256,
  ...
  "public_observatory_http_status":public_observatory_http_status,
  "public_aws_readiness_http_status":public_readiness_http_status,
  "shepherd":shepherd,
}
