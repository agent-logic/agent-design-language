...
validation_commands:
  - "`cargo fmt --manifest-path adl/Cargo.toml --check` passed. `cargo test --manifest-path adl/Cargo.toml --test codefriend_remediate` passed 4/4. `cargo test --manifest-path adl/Cargo.toml --test codefriend_synthesis` passed 5/5. `git diff --check` passed. These focused local checks prove deterministic remediation-plan generation/reading and synthesis regression scope; CI and independent exact-head review remain pending."
