...
name: "v0922-remediation-planner-execution-plan"
issue: 893
task_id: "issue-0893"
run_id: "issue-0893"
version: "0.92.2"
...
planned_pvf_lane_source: "https://github.com/agent-logic/agent-design-language/issues/893 deterministic production consumer acceptance; docs/validation/pvf_lanes.json"
...
estimate_source_ref: "https://github.com/agent-logic/agent-design-language/issues/893"
...
scope:
  ...
  out_of_scope:
    - "acceptance: `findings_to_actions_trace`, `bounded_action_acceptance`. pvf: `findings_to_actions_trace`, `bounded_action_acceptance`, `unrelated_action_rejected`, `source_mutation_rejected`, `action_plan_schema`. stop_conditions: `missing_required_input`, `required_proof_failed`, `scope_or_contract_conflict`, `required_proof_not_executed`, `partial_artifact_claimed_complete`, `source_mutation`, `unbounded_or_untraceable_action`. non_goals: `unrelated_scope`, `schema_or_scaffold_only_completion`, `security_tournament`, `autonomous_fixing`. Completion rejection: `plan_only`, `schema_only`, `scaffold_only`, `test_only_consumer`, `zero_executed_scenarios`. Canonical source: `docs/milestones/v0.92.2/WP_EXECUTION_SPECIFICATIONS_v0.92.2.yaml`, `WP_ISSUE_WAVE_v0.92.2.yaml`, `ATOMIC_TASK_CONTRACTS_v0.92.2.json`, and `ADOPTED_DESIGN_CONTRACTS_v0.92.2.md`. Use `docs/milestones/v0.92.2/CREATION_SELECTIONS_v0.92.2.md` and the adopted design contract: local `adl codefriend` in this repository, selected macOS/Linux qualification, shared registered OpenAI route with operator-supplied credential reference, and pinned Vector `410da89a0ed42c523143da89fffeb7f6402833e0` scope (seven dnsmsg-parser files plus three root manifest/license files; ten files/600 KiB total/400 KiB per file). Preserve both license notices and read-only source. Existing `adl/src/cli/mod.rs`, `cli/usage.rs` and `lib.rs` are registration owners. New paths are planned implementation, not a claim of existing product behavior. Preserve CF-EVIDENCE's versioned run/evidence/finding/publication semantics and immutable evidence; reuse its canonical test vectors and predecessor artifacts. Native v3 readiness, a dedicated bound FastWork worktree, an issue-bound goal and current path ownership precede implementation. Supporting tests, error handling and user documentation are part of this task. Record exact candidate/input/output identity and local versus required CI results. No schema/scaffold/unused helper/test-only caller/zero-execution result can close it. No autonomous source changes, paid calls, external publication or cloud resources are authorized merely by creation. Required real provider proof needs scoped execution authority and cannot be replaced by synthetic success."
constraints:
  - "design_time_plan_must_be_reviewed_before_execution"
  - "runtime_execution_must_update_spp_if_plan_changes"
  - "no_hidden_scope_expansion"
confidence: "medium"
plan_summary: "After accepted #892 and shared-owner recheck, freeze completed-synthesis input and traceability contracts; implement bounded evidence-linked remediation generation with one repair per action and acyclic ordering; wire the installed generator and complete plan reader; execute real synthesis fixtures and unrelated-action, unsupported-path, missing-evidence, cycle, invalid-input and source-mutation negatives; document limitations and obtain independent exact-head review."
assumptions:
  - "The linked source issue prompt, STP, and SIP remain the canonical design-time inputs."
proposed_steps:
  - id: "step-1"
    description: "Confirm dependency readiness and starting state: #892 (CF-SYNTHESIS) accepted merged output is required; it is not satisfied in this preparation."
    expected_output: ".csdlc/issues/893/cards/sip.md"
    allowed_mode: "design_review_then_execution"
  - id: "step-2"
    description: "Review repo inputs and scoped surfaces before editing: Full live source contract, retained without dropping requirements: # [v0.92.2][CF-REMEDIATE] Generate a bounded remediation plan from review findings ## One complete result The remediation planner consumes synthesized findings and produces dependency-ordered actionable repairs with ownership, evidence and acceptance criteria without changing source. Dependencies: CF-SYNTHESIS. Consume accepted merged predecessor output; logical IDs receive canonical issue numbers during creation. Batch membership adds no all-to-all gate. ## Production ownership Implement `plan remediation` behavior under the selected `adl codefriend` entrypoint. Proposed cohesive modules under `adl/src/codefriend/`: actions/remediation.rs. The CLI registration is `adl/src/cli/codefriend_cmd.rs` (CF-SHELL owns operator-control additions); coordinate shared wiring with predecessor owners. Add focused `adl/tests/codefriend_remediate.rs` and bounded fixture outputs. Exact flags are documented/tested during implementation; command wording denotes the selected behavior, not an already installed command. ## Acceptance and executed evidence 1. Consume an actual completed CF-SYNTHESIS result and generate a usable remediation plan: selected finding IDs, cited source evidence, one bounded repair per action, intended owner role or explicit unassigned status, exact relevant paths, dependencies, acceptance criteria, validation and risk/non-goals. 2. Produce an acyclic actionable repair order; distinguish shared causes from unrelated findings without bundling arbitrary work. Do not invent actual assignees, issue IDs or authority. Preserve unresolved uncertainty and omitted findings with reasons. 3. Execute finding-to-action tracing and schema checks on real synthesis output; negative cases reject unrelated actions, unsupported paths, untraceable acceptance and dependency cycles. The output reader must consume and display the complete plan through the installed product. 4. Prove inspected source and remote issue state remain unchanged. This task implements the planner, not the repairs or GitHub issue creation. A manually written plan or schema with no running generator does not close it. ## Shared execution boundary Use `docs/milestones/v0.92.2/CREATION_SELECTIONS_v0.92.2.md` and the adopted design contract: local `adl codefriend` in this repository, selected macOS/Linux qualification, shared registered OpenAI route with operator-supplied credential reference, and pinned Vector `410da89a0ed42c523143da89fffeb7f6402833e0` scope (seven dnsmsg-parser files plus three root manifest/license files; ten files/600 KiB total/400 KiB per file). Preserve both license notices and read-only source. Existing `adl/src/cli/mod.rs`, `cli/usage.rs` and `lib.rs` are registration owners. New paths are planned implementation, not a claim of existing product behavior. Preserve CF-EVIDENCE's versioned run/evidence/finding/publication semantics and immutable evidence; reuse its canonical test vectors and predecessor artifacts. Native v3 readiness, a dedicated bound FastWork worktree, an issue-bound goal and current path ownership precede implementation. Supporting tests, error handling and user documentation are part of this task. Record exact candidate/input/output identity and local versus required CI results. No schema/scaffold/unused helper/test-only caller/zero-execution result can close it. No autonomous source changes, paid calls, external publication or cloud resources are authorized merely by creation. Required real provider proof needs scoped execution authority and cannot be replaced by synthetic success. ## PVF and completion Declare each new test in a coupled proof inventory: deterministic local CPU contract/integration lane, isolated source/store and controlled clocks/transport, nonzero success and negative proof, bounded disk/process resources, required Beta feature and CF-INTEGRATE input gate. Provider-generated runs and browser/PDF visual review are separately identified execution/observation evidence, not deterministic guarantees. Run focused installed CLI and consumer tests plus required CI, then independent exact-head review. Redacted human diagnostics stay on stderr and machine output on stdout; test compatibility logging when exposed. Stop on unresolved owner/authority, inconsistent scope/provenance, failed or missing proof, source/credential exposure, stale approval or partial completion claimed as success. No unrelated feature work or customer-scale hosting. ## Inherited obligation ledger acceptance: `findings_to_actions_trace`, `bounded_action_acceptance`. pvf: `findings_to_actions_trace`, `bounded_action_acceptance`, `unrelated_action_rejected`, `source_mutation_rejected`, `action_plan_schema`. stop_conditions: `missing_required_input`, `required_proof_failed`, `scope_or_contract_conflict`, `required_proof_not_executed`, `partial_artifact_claimed_complete`, `source_mutation`, `unbounded_or_untraceable_action`. non_goals: `unrelated_scope`, `schema_or_scaffold_only_completion`, `security_tournament`, `autonomous_fixing`. Completion rejection: `plan_only`, `schema_only`, `scaffold_only`, `test_only_consumer`, `zero_executed_scenarios`. Canonical source: `docs/milestones/v0.92.2/WP_EXECUTION_SPECIFICATIONS_v0.92.2.yaml`, `WP_ISSUE_WAVE_v0.92.2.yaml`, `ATOMIC_TASK_CONTRACTS_v0.92.2.json`, and `ADOPTED_DESIGN_CONTRACTS_v0.92.2.md`. ## Canonical execution links Planning owner: #864. Creation/review batch: 4; this grouping adds no execution gate. Execution prerequisite: #892 (CF-SYNTHESIS); accepted output is required before dependent execution. Reviewed creation source: `54e5d8e100f39c644ca0aa03e3985ce66beb529e`. This issue records a complete task; creation does not claim execution or acceptance. <!-- csdlc-v3-operation:e62893108b62d1d3cbdc3da2dad80f5808d7dbbaa8fc232d4ced92edb0ea8f35 -->"
    expected_output: ".csdlc/issues/893/cards/stp.md"
    allowed_mode: "design_review_then_execution"
  - id: "step-3"
    description: "Implement only the bounded deliverables: Implement `plan remediation` behavior under the selected `adl codefriend` entrypoint. Proposed cohesive modules under `adl/src/codefriend/`: actions/remediation.rs. The CLI registration is `adl/src/cli/codefriend_cmd.rs` (CF-SHELL owns operator-control additions); coordinate shared wiring with predecessor owners. Add focused `adl/tests/codefriend_remediate.rs` and bounded fixture outputs. Exact flags are documented/tested during implementation; command wording denotes the selected behavior, not an already installed command. The remediation planner consumes synthesized findings and produces dependency-ordered actionable repairs with ownership, evidence and acceptance criteria without changing source. Dependencies: CF-SYNTHESIS. Consume accepted merged predecessor output; logical IDs receive canonical issue numbers during creation. Batch membership adds no all-to-all gate."
    expected_output: "tracked issue work product"
    allowed_mode: "execution_after_approval"
  - id: "step-4"
    description: "Run focused proof gates for acceptance: 1. Consume an actual completed CF-SYNTHESIS result and generate a usable remediation plan: selected finding IDs, cited source evidence, one bounded repair per action, intended owner role or explicit unassigned status, exact relevant paths, dependencies, acceptance criteria, validation and risk/non-goals. 2. Produce an acyclic actionable repair order; distinguish shared causes from unrelated findings without bundling arbitrary work. Do not invent actual assignees, issue IDs or authority. Preserve unresolved uncertainty and omitted findings with reasons. 3. Execute finding-to-action tracing and schema checks on real synthesis output; negative cases reject unrelated actions, unsupported paths, untraceable acceptance and dependency cycles. The output reader must consume and display the complete plan through the installed product. 4. Prove inspected source and remote issue state remain unchanged. This task implements the planner, not the repairs or GitHub issue creation. A manually written plan or schema with no running generator does not close it. acceptance: `findings_to_actions_trace`, `bounded_action_acceptance`. pvf: `findings_to_actions_trace`, `bounded_action_acceptance`, `unrelated_action_rejected`, `source_mutation_rejected`, `action_plan_schema`. stop_conditions: `missing_required_input`, `required_proof_failed`, `scope_or_contract_conflict`, `required_proof_not_executed`, `partial_artifact_claimed_complete`, `source_mutation`, `unbounded_or_untraceable_action`. non_goals: `unrelated_scope`, `schema_or_scaffold_only_completion`, `security_tournament`, `autonomous_fixing`. Completion rejection: `plan_only`, `schema_only`, `scaffold_only`, `test_only_consumer`, `zero_executed_scenarios`. Canonical source: `docs/milestones/v0.92.2/WP_EXECUTION_SPECIFICATIONS_v0.92.2.yaml`, `WP_ISSUE_WAVE_v0.92.2.yaml`, `ATOMIC_TASK_CONTRACTS_v0.92.2.json`, and `ADOPTED_DESIGN_CONTRACTS_v0.92.2.md`."
...
risks_and_edge_cases:
  - "acceptance: `findings_to_actions_trace`, `bounded_action_acceptance`. pvf: `findings_to_actions_trace`, `bounded_action_acceptance`, `unrelated_action_rejected`, `source_mutation_rejected`, `action_plan_schema`. stop_conditions: `missing_required_input`, `required_proof_failed`, `scope_or_contract_conflict`, `required_proof_not_executed`, `partial_artifact_claimed_complete`, `source_mutation`, `unbounded_or_untraceable_action`. non_goals: `unrelated_scope`, `schema_or_scaffold_only_completion`, `security_tournament`, `autonomous_fixing`. Completion rejection: `plan_only`, `schema_only`, `scaffold_only`, `test_only_consumer`, `zero_executed_scenarios`. Canonical source: `docs/milestones/v0.92.2/WP_EXECUTION_SPECIFICATIONS_v0.92.2.yaml`, `WP_ISSUE_WAVE_v0.92.2.yaml`, `ATOMIC_TASK_CONTRACTS_v0.92.2.json`, and `ADOPTED_DESIGN_CONTRACTS_v0.92.2.md`. Use `docs/milestones/v0.92.2/CREATION_SELECTIONS_v0.92.2.md` and the adopted design contract: local `adl codefriend` in this repository, selected macOS/Linux qualification, shared registered OpenAI route with operator-supplied credential reference, and pinned Vector `410da89a0ed42c523143da89fffeb7f6402833e0` scope (seven dnsmsg-parser files plus three root manifest/license files; ten files/600 KiB total/400 KiB per file). Preserve both license notices and read-only source. Existing `adl/src/cli/mod.rs`, `cli/usage.rs` and `lib.rs` are registration owners. New paths are planned implementation, not a claim of existing product behavior. Preserve CF-EVIDENCE's versioned run/evidence/finding/publication semantics and immutable evidence; reuse its canonical test vectors and predecessor artifacts. Native v3 readiness, a dedicated bound FastWork worktree, an issue-bound goal and current path ownership precede implementation. Supporting tests, error handling and user documentation are part of this task. Record exact candidate/input/output identity and local versus required CI results. No schema/scaffold/unused helper/test-only caller/zero-execution result can close it. No autonomous source changes, paid calls, external publication or cloud resources are authorized merely by creation. Required real provider proof needs scoped execution authority and cannot be replaced by synthetic success. #892 (CF-SYNTHESIS) accepted merged output is required; it is not satisfied in this preparation."
test_strategy:
...
notes: "#892 dependency was terminal before implementation. #893 is now bound in its FastWork worktree and implements only local remediation-plan generation/reading; no source repair, GitHub issue creation, external provider call, publication, or merge is claimed by this edit."
---
...
Design-time operative plan for `[v0.92.2][CF-REMEDIATE] Generate a bounded remediation plan from review findings`.
...
- Planning lane source: `https://github.com/agent-logic/agent-design-language/issues/893 deterministic production consumer acceptance; docs/validation/pvf_lanes.json`
...
- Estimate source ref: `https://github.com/agent-logic/agent-design-language/issues/893`
...
1. Confirm dependency readiness and starting state: #892 (CF-SYNTHESIS) accepted merged output is required; it is not satisfied in this preparation.
2. Review repo inputs and scoped surfaces before editing: Full live source contract, retained without dropping requirements: # [v0.92.2][CF-REMEDIATE] Generate a bounded remediation plan from review findings ## One complete result The remediation planner consumes synthesized findings and produces dependency-ordered actionable repairs with ownership, evidence and acceptance criteria without changing source. Dependencies: CF-SYNTHESIS. Consume accepted merged predecessor output; logical IDs receive canonical issue numbers during creation. Batch membership adds no all-to-all gate. ## Production ownership Implement `plan remediation` behavior under the selected `adl codefriend` entrypoint. Proposed cohesive modules under `adl/src/codefriend/`: actions/remediation.rs. The CLI registration is `adl/src/cli/codefriend_cmd.rs` (CF-SHELL owns operator-control additions); coordinate shared wiring with predecessor owners. Add focused `adl/tests/codefriend_remediate.rs` and bounded fixture outputs. Exact flags are documented/tested during implementation; command wording denotes the selected behavior, not an already installed command. ## Acceptance and executed evidence 1. Consume an actual completed CF-SYNTHESIS result and generate a usable remediation plan: selected finding IDs, cited source evidence, one bounded repair per action, intended owner role or explicit unassigned status, exact relevant paths, dependencies, acceptance criteria, validation and risk/non-goals. 2. Produce an acyclic actionable repair order; distinguish shared causes from unrelated findings without bundling arbitrary work. Do not invent actual assignees, issue IDs or authority. Preserve unresolved uncertainty and omitted findings with reasons. 3. Execute finding-to-action tracing and schema checks on real synthesis output; negative cases reject unrelated actions, unsupported paths, untraceable acceptance and dependency cycles. The output reader must consume and display the complete plan through the installed product. 4. Prove inspected source and remote issue state remain unchanged. This task implements the planner, not the repairs or GitHub issue creation. A manually written plan or schema with no running generator does not close it. ## Shared execution boundary Use `docs/milestones/v0.92.2/CREATION_SELECTIONS_v0.92.2.md` and the adopted design contract: local `adl codefriend` in this repository, selected macOS/Linux qualification, shared registered OpenAI route with operator-supplied credential reference, and pinned Vector `410da89a0ed42c523143da89fffeb7f6402833e0` scope (seven dnsmsg-parser files plus three root manifest/license files; ten files/600 KiB total/400 KiB per file). Preserve both license notices and read-only source. Existing `adl/src/cli/mod.rs`, `cli/usage.rs` and `lib.rs` are registration owners. New paths are planned implementation, not a claim of existing product behavior. Preserve CF-EVIDENCE's versioned run/evidence/finding/publication semantics and immutable evidence; reuse its canonical test vectors and predecessor artifacts. Native v3 readiness, a dedicated bound FastWork worktree, an issue-bound goal and current path ownership precede implementation. Supporting tests, error handling and user documentation are part of this task. Record exact candidate/input/output identity and local versus required CI results. No schema/scaffold/unused helper/test-only caller/zero-execution result can close it. No autonomous source changes, paid calls, external publication or cloud resources are authorized merely by creation. Required real provider proof needs scoped execution authority and cannot be replaced by synthetic success. ## PVF and completion Declare each new test in a coupled proof inventory: deterministic local CPU contract/integration lane, isolated source/store and controlled clocks/transport, nonzero success and negative proof, bounded disk/process resources, required Beta feature and CF-INTEGRATE input gate. Provider-generated runs and browser/PDF visual review are separately identified execution/observation evidence, not deterministic guarantees. Run focused installed CLI and consumer tests plus required CI, then independent exact-head review. Redacted human diagnostics stay on stderr and machine output on stdout; test compatibility logging when exposed. Stop on unresolved owner/authority, inconsistent scope/provenance, failed or missing proof, source/credential exposure, stale approval or partial completion claimed as success. No unrelated feature work or customer-scale hosting. ## Inherited obligation ledger acceptance: `findings_to_actions_trace`, `bounded_action_acceptance`. pvf: `findings_to_actions_trace`, `bounded_action_acceptance`, `unrelated_action_rejected`, `source_mutation_rejected`, `action_plan_schema`. stop_conditions: `missing_required_input`, `required_proof_failed`, `scope_or_contract_conflict`, `required_proof_not_executed`, `partial_artifact_claimed_complete`, `source_mutation`, `unbounded_or_untraceable_action`. non_goals: `unrelated_scope`, `schema_or_scaffold_only_completion`, `security_tournament`, `autonomous_fixing`. Completion rejection: `plan_only`, `schema_only`, `scaffold_only`, `test_only_consumer`, `zero_executed_scenarios`. Canonical source: `docs/milestones/v0.92.2/WP_EXECUTION_SPECIFICATIONS_v0.92.2.yaml`, `WP_ISSUE_WAVE_v0.92.2.yaml`, `ATOMIC_TASK_CONTRACTS_v0.92.2.json`, and `ADOPTED_DESIGN_CONTRACTS_v0.92.2.md`. ## Canonical execution links Planning owner: #864. Creation/review batch: 4; this grouping adds no execution gate. Execution prerequisite: #892 (CF-SYNTHESIS); accepted output is required before dependent execution. Reviewed creation source: `54e5d8e100f39c644ca0aa03e3985ce66beb529e`. This issue records a complete task; creation does not claim execution or acceptance. <!-- csdlc-v3-operation:e62893108b62d1d3cbdc3da2dad80f5808d7dbbaa8fc232d4ced92edb0ea8f35 -->
3. Implement only the bounded deliverables: Implement `plan remediation` behavior under the selected `adl codefriend` entrypoint. Proposed cohesive modules under `adl/src/codefriend/`: actions/remediation.rs. The CLI registration is `adl/src/cli/codefriend_cmd.rs` (CF-SHELL owns operator-control additions); coordinate shared wiring with predecessor owners. Add focused `adl/tests/codefriend_remediate.rs` and bounded fixture outputs. Exact flags are documented/tested during implementation; command wording denotes the selected behavior, not an already installed command. The remediation planner consumes synthesized findings and produces dependency-ordered actionable repairs with ownership, evidence and acceptance criteria without changing source. Dependencies: CF-SYNTHESIS. Consume accepted merged predecessor output; logical IDs receive canonical issue numbers during creation. Batch membership adds no all-to-all gate.
4. Run focused proof gates for acceptance: 1. Consume an actual completed CF-SYNTHESIS result and generate a usable remediation plan: selected finding IDs, cited source evidence, one bounded repair per action, intended owner role or explicit unassigned status, exact relevant paths, dependencies, acceptance criteria, validation and risk/non-goals. 2. Produce an acyclic actionable repair order; distinguish shared causes from unrelated findings without bundling arbitrary work. Do not invent actual assignees, issue IDs or authority. Preserve unresolved uncertainty and omitted findings with reasons. 3. Execute finding-to-action tracing and schema checks on real synthesis output; negative cases reject unrelated actions, unsupported paths, untraceable acceptance and dependency cycles. The output reader must consume and display the complete plan through the installed product. 4. Prove inspected source and remote issue state remain unchanged. This task implements the planner, not the repairs or GitHub issue creation. A manually written plan or schema with no running generator does not close it. acceptance: `findings_to_actions_trace`, `bounded_action_acceptance`. pvf: `findings_to_actions_trace`, `bounded_action_acceptance`, `unrelated_action_rejected`, `source_mutation_rejected`, `action_plan_schema`. stop_conditions: `missing_required_input`, `required_proof_failed`, `scope_or_contract_conflict`, `required_proof_not_executed`, `partial_artifact_claimed_complete`, `source_mutation`, `unbounded_or_untraceable_action`. non_goals: `unrelated_scope`, `schema_or_scaffold_only_completion`, `security_tournament`, `autonomous_fixing`. Completion rejection: `plan_only`, `schema_only`, `scaffold_only`, `test_only_consumer`, `zero_executed_scenarios`. Canonical source: `docs/milestones/v0.92.2/WP_EXECUTION_SPECIFICATIONS_v0.92.2.yaml`, `WP_ISSUE_WAVE_v0.92.2.yaml`, `ATOMIC_TASK_CONTRACTS_v0.92.2.json`, and `ADOPTED_DESIGN_CONTRACTS_v0.92.2.md`.
...
#892 dependency was terminal before implementation. #893 is now bound in its FastWork worktree and implements only local remediation-plan generation/reading; no source repair, GitHub issue creation, external provider call, publication, or merge is claimed by this edit.
