...
name: "native-typed-issue-close-execution-plan"
issue: 794
task_id: "issue-0794"
run_id: "issue-0794"
version: "1.0.4"
...
activation_state: "bound_execution_in_progress"
plan_revision: 1
...
scope:
  files:
    - "csdlc-v3 remote command path, CLI path, tests, docs/csdlc-v3/CONTRACT.md"
  components:
    - "native-typed-issue-close"
  out_of_scope:
    - "<non_goals_inline>"
constraints:
...
confidence: "medium"
plan_summary: "Add the smallest native close mutation beside existing issue create/comment/edit support; keep implementation finish separate by allowing only non-completion closure dispositions."
assumptions:
  - "The linked source issue prompt, STP, and SIP remain the canonical design-time inputs."
proposed_steps:
  - id: "step-1"
    description: "Confirm dependency readiness and starting state: C-SDLC v3 authority selector and existing GitHub mutation pipeline."
  ...
  - id: "step-2"
    ...
    allowed_mode: "design_review_then_execution"
  - id: "step-3"
    description: "Implement only the bounded deliverables: IssueClose enum/validation/invocation/readback, CLI close wrapper, focused tests, contract docs."
    expected_output: "tracked issue work product"
    allowed_mode: "execution_after_approval"
  - id: "step-4"
    description: "Run focused proof gates for acceptance: successful close/readback, idempotent replay, rejection cases, documented CLI, no raw gh/v2 default fallback."
...
risks_and_edge_cases:
  - "<risks_inline>"
test_strategy:
  - "fmt; focused issue_close tests; operational_cli_commands; remote tests; command_manifest; diff hygiene"
...
Design-time operative plan for `[v0.92.2][tooling] Add native typed duplicate and no-op issue closure`.

Add the smallest native close mutation beside existing issue create/comment/edit support; keep implementation finish separate by allowing only non-completion closure dispositions.
...
1. Confirm dependency readiness and starting state: C-SDLC v3 authority selector and existing GitHub mutation pipeline.
2. Review repo inputs and scoped surfaces before editing: <repo_inputs_inline>
3. Implement only the bounded deliverables: IssueClose enum/validation/invocation/readback, CLI close wrapper, focused tests, contract docs.
4. Run focused proof gates for acceptance: successful close/readback, idempotent replay, rejection cases, documented CLI, no raw gh/v2 default fallback.
