...
Implemented v3 GitHub issue-create parity, corrected post-cutover terminal finish operational-authority reporting, documented the real --execute issue-create path, recorded adjacent v3 defects as backlog issues, repaired executable install authority evidence, fixed read-only v3 diagnostic fallback for CI merge-checkout environments, and repaired executable remote authority so v3 GitHub mutations bind exact_review_sha to the checked-out Git HEAD while route-specific commands enforce issue-vs-PR mutation ownership.
...
- Bound executable remote authority to the actual checked-out Git HEAD by rejecting empty, malformed, unavailable, or mismatched exact_review_sha values before operational GitHub mutation dispatch.
- Changed command dispatch so github-issue accepts only issue mutations, github-pr accepts only PR mutations, and the generic github route remains the broad GitHub mutation envelope.
- Updated root csdlc help so github, github-issue, and github-pr advertise the implemented --execute switch alongside per-command help.
...
[
  {
    ...
    "purpose": "focused executable remote authority regression coverage, including active selector dispatch and forged exact_review_sha rejection",
    "outcome": "passed",
    "evidence_ref": "terminal output 2026-09-08 in bound worktree: 2 passed"
  },
  {
    "command": [
      "cargo",
      ...
      "executable_github_routes_reject_wrong_mutation_family_before_dispatch"
    ],
    "purpose": "focused CLI route ownership regression for github-issue and github-pr executable mutation families",
    "outcome": "passed",
  ...
  },
  {
    "command": [
      ...
      "--test",
      "command_manifest",
      "help_exposes_one_binary_command_surface"
    ],
    "purpose": "root help regression proving github-issue and github-pr advertise --execute",
    "outcome": "passed",
    "evidence_ref": "terminal output 2026-09-08 in bound worktree: 1 passed"
  },
  {
    ...
    "purpose": "CI-equivalent csdlc-v3 full test suite after authority-binding and route ownership repairs",
    "outcome": "passed",
