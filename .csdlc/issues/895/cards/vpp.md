...
notes: "Executed local proof covers installed prepare/approve/inspect/admit behavior; absolute-target approve/admit denial and final destination containment; exact canonical destination-root binding and wrong-root denial; anchor/store non-ancestry including the reserved anchor-directory store name; approve-to-invalidate and approve-to-withhold denial; copied-old-approval alternate-store replay denial; deleted revocation-tail detection; coherent same-store decision-plus-local-head rollback denial through an external head anchor; deterministic proof that invalidate and withhold cannot acquire the approval-store lock between head read and destination visibility; exact verified-byte snapshot publication without a second source read; concurrent-growth rejection and actual-byte aggregate accounting; binding mutations; incomplete runs; missing provenance; manifest/redaction/symlink/collision failures; evidence compatibility; review compatibility; formatting; strict warnings; and diff hygiene. Symlink/canonical checks are path based rather than directory-handle/no-follow semantics. The external anchor protects rollback of the decision store alone; full-machine rollback that also restores the external anchor is not claimed. A crash between local-head and external-head updates fails closed; automatic recovery is not claimed. No remote publication, provider, cloud, hosting, or Linux proof is claimed. Five independent reviews failed and their actionable findings are remediated locally; a distinct fresh review is required."
---
...
Executed local proof covers installed prepare/approve/inspect/admit behavior; absolute-target approve/admit denial and final destination containment; exact canonical destination-root binding and wrong-root denial; anchor/store non-ancestry including the reserved anchor-directory store name; approve-to-invalidate and approve-to-withhold denial; copied-old-approval alternate-store replay denial; deleted revocation-tail detection; coherent same-store decision-plus-local-head rollback denial through an external head anchor; deterministic proof that invalidate and withhold cannot acquire the approval-store lock between head read and destination visibility; exact verified-byte snapshot publication without a second source read; concurrent-growth rejection and actual-byte aggregate accounting; binding mutations; incomplete runs; missing provenance; manifest/redaction/symlink/collision failures; evidence compatibility; review compatibility; formatting; strict warnings; and diff hygiene. Symlink/canonical checks are path based rather than directory-handle/no-follow semantics. The external anchor protects rollback of the decision store alone; full-machine rollback that also restores the external anchor is not claimed. A crash between local-head and external-head updates fails closed; automatic recovery is not claimed. No remote publication, provider, cloud, hosting, or Linux proof is claimed. Five independent reviews failed and their actionable findings are remediated locally; a distinct fresh review is required.
