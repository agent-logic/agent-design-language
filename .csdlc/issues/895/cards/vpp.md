...
notes: "Executed local proof covers installed prepare/approve/inspect/admit behavior; exact canonical destination-root binding and wrong-root denial; approve-to-invalidate and approve-to-withhold denial; copied-old-approval alternate-store replay denial; deleted revocation-tail detection; coherent same-store decision-plus-local-head rollback denial through an external head anchor; deterministic proof that invalidate and withhold cannot acquire the approval-store lock between head read and destination visibility; exact verified-byte snapshot publication without a second source read; binding mutations; incomplete runs; missing provenance; manifest/redaction/symlink/collision failures; evidence compatibility; review compatibility; formatting; strict warnings; and diff hygiene. The external anchor protects rollback of the decision store alone; a full-machine rollback that also restores the external anchor is not claimed to be prevented by this local-only design. A crash between local-head and external-head updates fails closed; automatic recovery is not claimed. No executed claim is made for remote publication, provider, cloud, hosting, or Linux proof. Four independent reviews failed and their actionable findings are remediated locally; a distinct fresh review is required."
---
...
Executed local proof covers installed prepare/approve/inspect/admit behavior; exact canonical destination-root binding and wrong-root denial; approve-to-invalidate and approve-to-withhold denial; copied-old-approval alternate-store replay denial; deleted revocation-tail detection; coherent same-store decision-plus-local-head rollback denial through an external head anchor; deterministic proof that invalidate and withhold cannot acquire the approval-store lock between head read and destination visibility; exact verified-byte snapshot publication without a second source read; binding mutations; incomplete runs; missing provenance; manifest/redaction/symlink/collision failures; evidence compatibility; review compatibility; formatting; strict warnings; and diff hygiene. The external anchor protects rollback of the decision store alone; a full-machine rollback that also restores the external anchor is not claimed to be prevented by this local-only design. A crash between local-head and external-head updates fails closed; automatic recovery is not claimed. No executed claim is made for remote publication, provider, cloud, hosting, or Linux proof. Four independent reviews failed and their actionable findings are remediated locally; a distinct fresh review is required.
