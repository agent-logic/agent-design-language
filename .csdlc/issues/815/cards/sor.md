...
Card Status: ready
Status: implemented
Generated: 2026-09-09T21:20:00Z
...
Cloud mutation approval now requires an externally anchored operator signature; AWS additionally binds authenticated account identity, actual tfplan bytes, its digest sidecar, and a Terraform-derived reviewed projection.
...
- Verification scope: `#727 AWS authorization validator and template/runbook; #731 GCP authorization validator and mutation preflight; shared #815 verifier, canonical signature contract, GCP template, and deterministic local fixtures.`
- Integration method used: `<integration_method_used>`
- Verification performed:
  - `<integration_verification_command>`
    `<integration_verification_effect>`
- Result: `Implementation and local proof are tracked on codex/815-cloud-authorization-authenticity in the bound FastWork worktree; PR publication and merge are not yet claimed.`
...
- Validation commands and their purpose:
  - `<validation_command>`
    `Proves editable JSON and mutable digest sidecars cannot independently grant mutation authority, provider identity and exact plan bytes are bound, every required tamper/replay case fails before provider invocation, and read-only proof remains usable without approval.`
- Results:
  - `The #815 no-cloud matrix passed for valid signed packets and rejected unsigned/forged approval, changed plan bytes, changed digest sidecar, changed JSON, Terraform projection drift, expiry, and cross-account/project replay. GCP provider-call preflight, AWS/GCP read-only availability, Python compilation, shell syntax, native six-card validation, and diff hygiene passed.`
