...
- Extended the existing two-node GCP module with optional prepared disk attachment while preserving its default issue #509 behavior.
- Added separate preparation, snapshot-catalog, and launch Terraform roots so idle state retains only Runtime and Ollama/model snapshots and ordinary launch teardown cannot delete them.
...
[
  ...
  },
  {
    "command": [
      "terraform",
      "-chdir=infra/gcp/workloads/warm-polis/preparation",
      "test"
    ],
    "purpose": "Prove concurrent disk hydration and removal of temporary preparation VMs while staging disks are retained only until snapshot verification completes.",
    "outcome": "passed",
    "evidence_ref": "preparation-terraform.log"
  },
  {
    "command": [
      "terraform",
      "-chdir=infra/gcp/workloads/warm-polis/snapshot-catalog",
      "test"
    ],
    "purpose": "Prove two versioned snapshots, restored-content verification, and a snapshots-only retained steady state.",
    "outcome": "passed",
    "evidence_ref": "snapshot-catalog-terraform.log"
  },
  {
    "command": [
      "bash",
