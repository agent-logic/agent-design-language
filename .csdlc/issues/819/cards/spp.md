...
confidence: "medium"
plan_summary: "Freeze the exact 152-row denominator; map each row to an executed proof or explicit governed amendment; run and retain candidate-bound proof; validate positive and negative reconciliation; obtain exact-head review; publish."
assumptions:
...
proposed_steps:
  ...
  - id: "step-2"
    ...
    allowed_mode: "design_review_then_execution"
  - id: "step-3"
    description: "Implement only the bounded deliverables: Resolution plan, candidate-bound command logs and reconciliation receipt, strict validator, ten-case negative matrix, and truthful lifecycle evidence."
    expected_output: "tracked issue work product"
    allowed_mode: "execution_after_approval"
  - id: "step-4"
    description: "Run focused proof gates for acceptance: Consume 152 unique rows once; bind proving evidence to the candidate; keep amendments explicitly non-pass; reject missing, duplicate, unresolved, failed, stale, or synthetic-pass receipts; exact-head review."
...
risks_and_edge_cases:
  - "A broad passing suite is non-proving unless each execution row names an observed test; an amendment must never be reported as a behavioral pass; candidate bytes and logs can drift."
test_strategy:
  - "Build the exact plan; run csdlc-v3 tests, all-target clippy, and the current V3-A contract validator; validate all 152 receipts; run ten negative mutations; run diff and publication-path hygiene."
...
notes: "Fail closed on denominator drift, weak row-to-test mapping, stale candidate bytes, missing evidence, or amendment overclaim."
---
...
Freeze the exact 152-row denominator; map each row to an executed proof or explicit governed amendment; run and retain candidate-bound proof; validate positive and negative reconciliation; obtain exact-head review; publish.
...
3. Implement only the bounded deliverables: Resolution plan, candidate-bound command logs and reconciliation receipt, strict validator, ten-case negative matrix, and truthful lifecycle evidence.
4. Run focused proof gates for acceptance: Consume 152 unique rows once; bind proving evidence to the candidate; keep amendments explicitly non-pass; reject missing, duplicate, unresolved, failed, stale, or synthetic-pass receipts; exact-head review.
...
Fail closed on denominator drift, weak row-to-test mapping, stale candidate bytes, missing evidence, or amendment overclaim.
