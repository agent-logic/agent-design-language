...
confidence: "medium"
plan_summary: "Model removal with a durable authoritative intent/state; make roster and tombstone writes recoverable; reconcile partial states on startup; update in-memory projections only after durable convergence; prove each crash boundary."
assumptions:
...
Design-time operative plan for `[v0.92.1][TAIL-06.01][runtime] Make dynamic-agent removal crash-consistent`.

Model removal with a durable authoritative intent/state; make roster and tombstone writes recoverable; reconcile partial states on startup; update in-memory projections only after durable convergence; prove each crash boundary.
