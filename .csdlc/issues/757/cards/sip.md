...
Context:
- Issue: https://github.com/agent-logic/agent-design-language/issues/757
- PR:
...
Every interrupted removal converges after restart to a coherent resident agent with recoverable state or an authoritatively absent agent.
...
adl-runtime-kernel/src/control.rs; adl-runtime-kernel/src/agent_partial_checkpoint.rs; narrowly coupled Runtime tests and issue evidence.

## Validation Plan

Deterministic failure injection at each durable boundary, restart reconciliation tests, focused Runtime owner lane, diff hygiene and independent pre-PR review.
...
No unrelated checkpoint serialization redesign, provider changes, or broad Runtime refactor.
