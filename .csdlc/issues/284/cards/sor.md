...
- Repaired .csdlc/evidence/284/validate_adr0066_guardian_authority_evidence.sh and the prepared mirror so linked-worktree terminal-cache lookup uses the current Git common directory and validates the retained-terminal schema without hard-coding the primary checkout.
- Repaired #284 VPP/SOR typed validation lane so it uses the stable generated owner binary with worktree-relative --root . rather than hard-coding the issue worktree path.
...
[
  ...
  },
  {
    ...
    "purpose": "Run typed C-SDLC v2 issue validation from the issue worktree with the stable generated owner binary.",
    "outcome": "passed",
