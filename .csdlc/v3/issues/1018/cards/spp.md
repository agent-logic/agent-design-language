...
source_refs:
  - kind: "issue"
    ref: "https://github.com/agent-logic/agent-design-language/issues/1018"
...
scope:
  files:
    - "csdlc-v3 remote PR-create recovery implementation and the smallest focused regression tests, plus issue-local lifecycle/evidence records. No unrelated lifecycle or product changes."
  components:
    - "<slug>"
  out_of_scope:
    - "<non_goals_inline>"
...
proposed_steps:
  ...
  - id: "step-2"
    description: "Review repo inputs and scoped surfaces before editing: Issue #1018; csdlc-v3 semantic remote mutation intent, recovery and publication owners; retained PR-create recovery tests and fixtures; current operator manuals and schemas."
...
risks_and_edge_cases:
  - "<risks_inline>"
test_strategy:
  - "Run focused csdlc-v3 remote mutation, semantic intent and installed command tests covering PR-create recovery; verify failure before any effect for wrong head, mismatched PR and stale authority; prove idempotent single-effect behavior and repeated recovery; run formatting, Clippy for the touched crate, diff hygiene, independent exact-head review and required CI."
...
Design-time operative plan for `[v0.92.2][C-SDLC] Recover PR creation after a consumed authenticated-absence retry`.
