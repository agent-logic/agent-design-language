...
fn live_black_box_parity_classification_covers_matrix_without_counting_blockers_as_passed() {
    ...
    assert_eq!(
        classification["summary"]["runtime_v3_fixture_only"].as_u64(),
        Some(
            *disposition_counts
                .get("runtime_v3_fixture_only")
                .unwrap_or(&0) as u64
        )
    );
    assert_eq!(
...
fn kernel_lifecycle_proof_resolves_only_kernel_lifecycle_blocker() {
    ...
    assert_eq!(proof["disposition"], "runtime_v3_fixture_only");
    ...
    let kernel = classification["capabilities"]
        ...
        .unwrap();
    assert_eq!(kernel["disposition"], "runtime_v3_fixture_only");
    ...
    assert_eq!(classification["summary"]["live_equivalent_fixture"], 1);
    assert_eq!(classification["cutover_eligible"], false);
}

#[test]
fn topology_backpressure_proof_resolves_only_topology_backpressure_blocker() {
    let proof: serde_json::Value = serde_json::from_str(include_str!(
    ...
    assert_eq!(
        proof["schema"],
        "adl.runtime_v3.topology_backpressure_proof.v1"
    );
    assert_eq!(proof["issue"], 5278);
    assert_eq!(proof["sprint_issue"], 5276);
    assert_eq!(proof["capability"], "kernel.topology_and_backpressure");
    assert_eq!(proof["disposition"], "runtime_v3_fixture_only");
    assert_eq!(proof["runtime_v2_default_preserved"], true);
    assert_eq!(proof["runtime_v2_internal_reuse"], false);
    assert_eq!(proof["default_runtime_switch_authorized"], false);
    ...
    assert_eq!(
        proof["acceptance"]["blocked_or_deferred_counts_as_passed"],
        false
    );
    for behavior in [
        "dependency_ordering",
        ...
        "cycle_rejection",
        "typed_port_mismatch_rejection",
        "canonical_topology_configuration",
        "bounded_channel_saturation",
        "resource_pressure_stop",
        "portable_weather_observation",
        "no_unbounded_disk_backpressure",
    ] {
        assert_eq!(
            proof["behaviors"][behavior]["status"], "proved",
            "missing topology/backpressure proof behavior {behavior}"
        );
    }

    let topology = classification["capabilities"]
        .as_array()
        .unwrap()
        .iter()
        .find(|capability| capability["id"] == "kernel.topology_and_backpressure")
        .unwrap();
    assert_eq!(topology["disposition"], "runtime_v3_fixture_only");
...
#[test]
fn service_contracts_configuration_proof_resolves_only_service_contracts_blocker() {
    let proof: serde_json::Value = serde_json::from_str(include_str!(
    ...
    assert_eq!(
        proof["schema"],
        "adl.runtime_v3.service_contracts_configuration_proof.v1"
    );
    assert_eq!(proof["issue"], 5279);
    assert_eq!(proof["sprint_issue"], 5276);
    assert_eq!(proof["capability"], "service.contracts_and_configuration");
    assert_eq!(proof["disposition"], "runtime_v3_fixture_only");
    assert_eq!(proof["runtime_v2_default_preserved"], true);
    assert_eq!(proof["runtime_v2_internal_reuse"], false);
    assert_eq!(proof["default_runtime_switch_authorized"], false);
    ...
    assert_eq!(proof["acceptance"]["direct_runtime_v3_proof"], true);
    for behavior in [
        "service_contract_schema_validation",
        "component_surface_binding",
        "capability_matching",
        "fail_closed_capability_requirements",
        "duplicate_contract_rejection",
        "configuration_schema_rejection",
        "canonical_configuration_projection",
        "factory_registry_fail_closed",
        "deterministic_topology_build",
    ] {
        assert_eq!(
            proof["behaviors"][behavior]["status"], "proved",
            "missing service-contract/configuration proof behavior {behavior}"
        );
    }

    let service_contracts = classification["capabilities"]
        .as_array()
        .unwrap()
        .iter()
        .find(|capability| capability["id"] == "service.contracts_and_configuration")
        .unwrap();
    assert_eq!(service_contracts["disposition"], "runtime_v3_fixture_only");
    assert!(service_contracts.get("blocking_issue").is_none());
    assert!(service_contracts["proof"]
        .as_str()
        .unwrap()
        .contains("#5279"));
    assert_eq!(classification["summary"]["blocker"], 0);
...
#[test]
fn continuity_replay_recovery_proof_resolves_only_continuity_blocker() {
    let proof: serde_json::Value = serde_json::from_str(include_str!(
    ...
    assert_eq!(
        proof["schema"],
        "adl.runtime_v3.continuity_replay_recovery_proof.v1"
    );
    assert_eq!(proof["issue"], 5280);
    assert_eq!(proof["sprint_issue"], 5276);
    assert_eq!(proof["capability"], "continuity.replay_recovery");
    assert_eq!(proof["disposition"], "runtime_v3_fixture_only");
    assert_eq!(proof["runtime_v2_default_preserved"], true);
    assert_eq!(proof["runtime_v2_internal_reuse"], false);
    assert_eq!(proof["default_runtime_switch_authorized"], false);
    ...
    assert_eq!(proof["acceptance"]["direct_runtime_v3_proof"], true);
    for behavior in [
        "checkpoint_quiesce_before_snapshot",
        "bounded_parallel_serialization",
        "signed_manifest_and_integrity",
        "corrupt_snapshot_quarantine",
        "identity_schema_refusal",
        "missing_checkpoint_restart_fresh",
        "replay_prefix_validation",
        "graceful_resource_stop_serialization",
        "fresh_kernel_generation_continuity",
        "restore_continuity_after_checkpoint",
    ] {
        assert_eq!(
            proof["behaviors"][behavior]["status"], "proved",
            "missing continuity/replay proof behavior {behavior}"
        );
    }

    let continuity = classification["capabilities"]
        .as_array()
        .unwrap()
        .iter()
        .find(|capability| capability["id"] == "continuity.replay_recovery")
        .unwrap();
    assert_eq!(continuity["disposition"], "runtime_v3_fixture_only");
...
#[test]
fn adaptive_learning_dag_proof_resolves_only_learning_blocker() {
    let proof: serde_json::Value = serde_json::from_str(include_str!(
    ...
    assert_eq!(
        proof["schema"],
        "adl.runtime_v3.adaptive_learning_dag_proof.v1"
    );
    assert_eq!(proof["issue"], 5281);
    assert_eq!(proof["sprint_issue"], 5276);
    assert_eq!(proof["capability"], "learning.adaptive_dag");
    assert_eq!(proof["disposition"], "runtime_v3_fixture_only");
    assert_eq!(proof["runtime_v2_default_preserved"], true);
    assert_eq!(proof["runtime_v2_internal_reuse"], false);
    assert_eq!(proof["default_runtime_switch_authorized"], false);
    ...
    assert_eq!(proof["acceptance"]["direct_runtime_v3_proof"], true);
    for behavior in [
        "bounded_canonical_graph_validation",
        "deterministic_graph_execution",
        "bounded_adaptive_loop",
        "replay_resume_integrity",
        "checkpointed_adaptation_state",
        "policy_governed_graph_mutation",
        "mutation_evidence_durability",
        "mutation_authority_fail_closed",
        "typed_reasoning_topology",
    ] {
        assert_eq!(
            proof["behaviors"][behavior]["status"], "proved",
            "missing adaptive-learning proof behavior {behavior}"
        );
    }

    let learning = classification["capabilities"]
        .as_array()
        .unwrap()
        .iter()
        .find(|capability| capability["id"] == "learning.adaptive_dag")
        .unwrap();
    assert_eq!(learning["disposition"], "runtime_v3_fixture_only");
...
#[test]
fn governance_freedom_gate_aee_proof_resolves_only_governance_blocker() {
    let proof: serde_json::Value = serde_json::from_str(include_str!(
    ...
    assert_eq!(
        proof["schema"],
        "adl.runtime_v3.governance_freedom_gate_aee_proof.v1"
    );
    assert_eq!(proof["issue"], 5282);
    assert_eq!(proof["sprint_issue"], 5276);
    assert_eq!(proof["capability"], "governance.freedom_gate_aee");
    assert_eq!(proof["disposition"], "runtime_v3_fixture_only");
    assert_eq!(proof["runtime_v2_default_preserved"], true);
    assert_eq!(proof["runtime_v2_internal_reuse"], false);
    assert_eq!(proof["default_runtime_switch_authorized"], false);
    ...
    assert_eq!(proof["acceptance"]["direct_runtime_v3_proof"], true);
    for behavior in [
        "signed_gate_before_actuation",
        "fail_closed_admission",
        "delegation_attenuation",
        "revocation_and_resource_refusal",
        "appeal_and_operator_review",
        "bounded_actuation_quarantine",
        "cancellation_checkpoint_safety",
        "checkpointed_governance_recovery",
        "typed_governance_topology",
    ] {
        assert_eq!(
            proof["behaviors"][behavior]["status"], "proved",
            "missing governance proof behavior {behavior}"
        );
    }

    let governance = classification["capabilities"]
        .as_array()
        .unwrap()
        .iter()
        .find(|capability| capability["id"] == "governance.freedom_gate_aee")
        .unwrap();
    assert_eq!(governance["disposition"], "runtime_v3_fixture_only");
...
fn delegation_resources_proof_resolves_only_delegation_blocker() {
    ...
    let delegation = classification["capabilities"]
        ...
        .unwrap();
    assert_eq!(delegation["disposition"], "runtime_v3_fixture_only");
...
#[test]
fn acip_a2a_cloud_network_proof_resolves_final_live_black_box_blocker() {
    let proof: serde_json::Value = serde_json::from_str(include_str!(
    ...
    assert_eq!(
        proof["schema"],
        "adl.runtime_v3.acip_a2a_cloud_network_proof.v1"
    );
    assert_eq!(proof["issue"], 5285);
    assert_eq!(proof["sprint_issue"], 5276);
    assert_eq!(proof["capability"], "network.acip_a2a_cloud");
    assert_eq!(proof["disposition"], "runtime_v3_fixture_only");
    assert_eq!(proof["runtime_v2_default_preserved"], true);
    assert_eq!(proof["runtime_v2_internal_reuse"], false);
    assert_eq!(proof["default_runtime_switch_authorized"], false);
    assert_eq!(proof["cutover_eligible"], false);
    assert_eq!(
        proof["acceptance"]["runtime_v2_decommission_authorized"],
        false
    );
    assert_eq!(proof["acceptance"]["direct_runtime_v3_proof"], true);
    for behavior in [
        "local_control_api_isolation",
        "signed_control_route",
        "control_port_contract",
        "graceful_network_shutdown",
        "stdout_stderr_observability_separation",
        "cloudwatch_vector_boundary",
        "network_weather_counters",
        "safe_cloud_non_claims",
        "gpu_and_remote_lanes_not_counted_as_network_pass",
    ] {
        assert_eq!(
            proof["behaviors"][behavior]["status"], "proved",
            "missing ACIP/A2A/cloud network proof behavior {behavior}"
        );
    }

    let network = classification["capabilities"]
        .as_array()
        .unwrap()
        .iter()
        .find(|capability| capability["id"] == "network.acip_a2a_cloud")
        .unwrap();
    assert_eq!(network["disposition"], "runtime_v3_fixture_only");
    assert!(network.get("blocking_issue").is_none());
    assert!(network["proof"].as_str().unwrap().contains("#5285"));
    assert_eq!(classification["summary"]["blocker"], 0);
    assert_eq!(classification["summary"]["live_equivalent_fixture"], 1);
    assert_eq!(classification["cutover_eligible"], false);
    assert_eq!(classification["default_runtime_switch_authorized"], false);
...
#[test]
fn final_cutover_decision_keeps_v2_default_after_live_parity_clear() {
    ...
    assert_ne!(
        ...
        "the immutable decision-time snapshot must remain distinct from the corrected current classification"
    );
    assert_eq!(
    ...
    assert!(!blockers.contains("kernel.topology_and_backpressure"));
    assert!(!blockers.contains("service.contracts_and_configuration"));
    ...
    assert!(!blockers.contains("network.acip_a2a_cloud"));
...
fn release_proof_gate_closes_without_authorizing_default_cutover() {
    ...
    for release_gate in gate["gates"].as_array().unwrap() {
        ...
        if satisfies_live_requirement {
            assert!(
                matches!(evidence_class, "executed" | "live_local_https"),
                "non-live evidence cannot satisfy live requirement {}",
                release_gate["id"]
            );
        }
    }
    ...
    assert_eq!(
        closed_children,
        [
            5247, 5248, 5249, 5250, 5251, 5252, 5253, 5254, 5277, 5278, 5279, 5280, 5281, 5282,
            5283, 5284, 5285, 5286,
        ]
        .into_iter()
        .collect()
...
fn observatory_consumes_runtime_v3_only_by_explicit_opt_in() {
    ...
    assert_eq!(
        ...
        "demos/html-observatory/?runtime=v3&runtimeApiBase=https://<runtime-gateway-host>&live=1"
    );
    assert_eq!(
