...
impl OperatorAttentionInbox {
    ...
    pub fn submit(
        ...
        let request = OperatorAttentionRequest {
            ...
            source_principal_id: input.source_identity.principal_id.clone(),
            display_name: input.source_identity.display_name.clone(),
            reason: input.reason,
    ...
    fn validate_input(
        ...
        if !input.source_identity.can_request_attention {
            return Err(OperatorAttentionError::UnauthorizedSource);
        }
        if input.priority == OperatorAttentionPriority::Urgent
            && !input.source_identity.can_mark_urgent
        {
            return Err(OperatorAttentionError::UnauthorizedUrgency);
        }
        if input.source_identity.principal_id.trim().is_empty() {
            return Err(OperatorAttentionError::InvalidRequest(
