...
fn wrong_purpose_revoked_and_unapproved_certificates_are_rejected() {
    ...
    let certificates = open_certificates(&temp, &root);
    certificates
        .activate(&TEST_CERTIFICATE_STORE_ACCESS, &advertisement_certificate, NOW)
        .unwrap();
    let weather = open_weather(&temp, 8);
...
fn freshness_lifetime_and_normalization_bounds_are_explicit() {
    ...
    let certificates = open_certificates(&temp, &root);
    certificates.activate(&TEST_CERTIFICATE_STORE_ACCESS, &certificate, NOW).unwrap();
    let weather = open_weather(&temp, 8);
...
fn certificate_window_revocation_rotation_and_refresh_bound_cached_weather() {
    ...
    let certificates = open_certificates(&temp, &root);
    certificates.activate(&TEST_CERTIFICATE_STORE_ACCESS, &certificate_a, NOW).unwrap();
    let weather = open_weather(&temp, 8);
    weather
        .admit(
            signed(
    ...
    certificates.activate(&TEST_CERTIFICATE_STORE_ACCESS, &certificate_b, NOW + 1).unwrap();
    assert_eq!(
        weather
    ...
    revoked_certificates.activate(&TEST_CERTIFICATE_STORE_ACCESS, &certificate_a, NOW).unwrap();
...
fn holder_capacity_payload_and_durable_corruption_fail_closed() {
    ...
    let certificates = open_certificates(&temp, &root);
    certificates.activate(&TEST_CERTIFICATE_STORE_ACCESS, &certificate_a, NOW).unwrap();
    certificates.activate(&TEST_CERTIFICATE_STORE_ACCESS, &certificate_b, NOW).unwrap();
    {
