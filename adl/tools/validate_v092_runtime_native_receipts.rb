...
def checked_file(path, digest, label, allow_empty: false)
  ...
  abort "#{label} path must be issue-local" unless path.to_s.start_with?(prefix)
...
packet_path = ARGV.fetch(0, ".csdlc/evidence/#{ISSUE}/runtime-native-receipts.json")
abort "missing #{packet_path}" unless File.file?(packet_path)
...
receipts.each do |receipt|
  ...
  artifacts.each do |artifact|
    checked_file(artifact.fetch("path"), artifact.fetch("sha256"), "#{platform} artifact")
  end

  assertions = Array(receipt["assertions"])
  abort "#{platform} assertion denominator drift" unless assertions.map { |entry| entry["name"] }.sort == ASSERTIONS.sort
