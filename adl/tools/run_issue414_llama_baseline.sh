...
for i in (1,2): residents.append({"agent_id":f"resident-{i}","provider_id":"local_ollama","model":"llama3.1:8b","artifact_sha256":sys.argv[4],"quantization":sys.argv[5],"configuration_sha256":sys.argv[6],"completed_task_sha256":(t/f"cold-{i}.sha").read_text().strip(),"continuation_request_sha256":(t/f"warm-{i}.request.sha").read_text().strip()})
