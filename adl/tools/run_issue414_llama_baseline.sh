...
trap cleanup EXIT
command -v curl >/dev/null; command -v jq >/dev/null; [[ -x "$BIN" ]]
...
python3 - "$TMP" "$runtime" "$build" "$artifact" "$quant" "$config" >"$TMP/input.json" <<'PY'
...
print(json.dumps({"residents":residents,"existing_agent_specs":[],"retained_runtime_root":sys.argv[2],"build_cache_root":sys.argv[3],"runtime_volume_identity_sha256":__import__("hashlib").sha256(b"reference-volume").hexdigest(),"source_host":"external-reference","target_host":"local"},sort_keys=True))
PY
