...
case "$1" in
  ...
  status)
    if [[ "${ADL_ISSUE268_FAKE_MANAGER_STATE:-dead}" == active ]]; then
      printf 'status=running run_id=issue268-six-hour-r7i-20260821-61\n'
      exit 0
    fi
    printf 'status=incomplete run_id=issue268-six-hour-r7i-20260821-61 action=inspect_logs_or_cleanup\n'
    exit 1
    ;;
  cleanup) exit 0 ;;
  preflight)
...
assert request["request_id"] == "issue268-six-hour-r7i-20260821-61"
...
grep -F -- '--runtime-continuity-volume-name issue268-six-hour-r7i-20260821-61-runtime' "$test_root/owner.log" >/dev/null
...
grep -F 'cloudformation describe-stacks --stack-name adl-issue268-runtime-61' "$test_root/aws.log" >/dev/null
[[ ! -s "$test_root/owner.log" ]]
...
[[ $(grep -c 'Name=tag:adl:run_id,Values=issue268-six-hour-r7i-20260821-61' "$test_root/aws.log") == 1 ]]
: >"$test_root/aws.log"

cat >"$test_root/summary.json" <<'EOF'
{
  "issue": 268,
  "run_id": "issue268-six-hour-r7i-20260821-61",
  "status": "passed",
