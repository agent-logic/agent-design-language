...
case "$1" in
  ...
  status)
    if [[ "${ADL_ISSUE268_FAKE_MANAGER_STATE:-dead}" == active ]]; then
      printf 'status=running run_id=issue268-six-hour-r7i-20260819-25\n'
      exit 0
    fi
    printf 'status=incomplete run_id=issue268-six-hour-r7i-20260819-25 action=inspect_logs_or_cleanup\n'
    exit 1
    ;;
  cleanup) exit 0 ;;
...
assert request["request_id"] == "issue268-six-hour-r7i-20260819-25"
...
[[ $(grep -c 'Name=tag:adl:run_id,Values=issue268-six-hour-r7i-20260819-25' "$test_root/aws.log") == 2 ]]
: >"$test_root/aws.log"

cat >"$test_root/summary.json" <<'EOF'
{
  "issue": 268,
  "run_id": "issue268-six-hour-r7i-20260819-25",
  "status": "passed",
...
[[ $(grep -c 'Name=tag:adl:run_id,Values=issue268-six-hour-r7i-20260819-25' "$test_root/aws.log") == 1 ]]
