...
if "trust_roots_match = re.search" not in v or "ssl.create_default_context(cafile=trust_roots)" not in v:
    raise SystemExit("authenticated probe must use configured CA trust roots")
if "observed_error_types=" not in v:
    raise SystemExit("authenticated probe must retain bounded cross-platform failure classes")
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    if (
        ...
        or 'observatory_public_origin = "https://observatory.example.test"' not in localized
        or '  "https://observatory.example.test",' not in localized
    ):
    ...
    source.write_text(canonical.replace('observatory_public_origin = "https://observatory.dev.agent-logic.ai"\n', '', 1))
    if subprocess.run(snippet_args, input=snippet, text=True, capture_output=True).returncode == 0:
        raise SystemExit("missing polis Observatory origin unexpectedly localized")
    source.write_text(canonical + 'observatory_public_origin = "https://duplicate.invalid"\n')
    if subprocess.run(snippet_args, input=snippet, text=True, capture_output=True).returncode == 0:
        raise SystemExit("duplicate polis Observatory origins unexpectedly localized")
...
case "$1" in
  ...
  status)
    if [[ "${ADL_ISSUE268_FAKE_MANAGER_STATE:-dead}" == active ]]; then
      printf 'status=running run_id=issue268-six-hour-r7i-20260821-72\n'
      exit 0
    fi
    printf 'status=incomplete run_id=issue268-six-hour-r7i-20260821-72 action=inspect_logs_or_cleanup\n'
    exit 1
    ;;
  cleanup) exit 0 ;;
  preflight)
...
assert request["request_id"] == "issue268-six-hour-r7i-20260821-72"
...
grep -F -- '--runtime-continuity-volume-name issue268-six-hour-r7i-20260821-72-runtime' "$test_root/owner.log" >/dev/null
...
grep -F 'cloudformation describe-stacks --stack-name adl-issue268-runtime-72' "$test_root/aws.log" >/dev/null
[[ ! -s "$test_root/owner.log" ]]
...
[[ $(grep -c 'Name=tag:adl:run_id,Values=issue268-six-hour-r7i-20260821-72' "$test_root/aws.log") == 1 ]]
: >"$test_root/aws.log"

cat >"$test_root/summary.json" <<'EOF'
{
  "issue": 268,
  "run_id": "issue268-six-hour-r7i-20260821-72",
  "status": "passed",
...
grep -Fq '.parameters("executionTimeout", vec![ssm_execution_timeout_seconds])' "$ROOT/tools/aws_remote_validation/src/aws_remote_validation.rs"
