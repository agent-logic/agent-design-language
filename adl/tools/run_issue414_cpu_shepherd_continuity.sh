...
if [[ "$HOST_CLASS" == r7i.2xlarge ]]; then
  ...
  cpu_count="$(getconf _NPROCESSORS_ONLN 2>/dev/null || sysctl -n hw.logicalcpu)"
...
for model in "${MODELS[@]}"; do
  ...
  jq -n --arg model "$model" --arg prompt "$prompt" '{model:$model,prompt:$prompt,stream:false,format:"json",options:{num_ctx:8192,temperature:0,seed:414}}' >"$tmp/request.json"
  ...
  for phase in cold warm; do
    jq -e '.done == true and (.response | fromjson | .next_action == "verify_population_digest") and (.response | fromjson | .ordered_steps | length == 3)' "$tmp/$model.$phase.json" >/dev/null || {
      echo "$model $phase response was not useful schema-valid work" >&2
      exit 67
    }
    jq -r '.response | fromjson | @json' "$tmp/$model.$phase.json" | shasum -a 256 | awk '{print $1}' >"$tmp/$model.$phase.sha256"
  done
done
