...
EXAMPLE="adl/examples/v0-7-enterprise-signed-remote.adl.yaml"
...
run_success() {
  umask 077
  cargo run -q --manifest-path adl/Cargo.toml --bin adl -- keygen --out-dir "$KEY_DIR"
  ...
  ADL_OLLAMA_BIN=adl/tools/mock_ollama_v0_4.sh \
    cargo run -q --manifest-path adl/Cargo.toml --bin adl-remote -- 127.0.0.1:8787 >"$REMOTE_LOG" 2>&1 &
  REMOTE_PID=$!
  sleep 1

  ADL_OLLAMA_BIN=adl/tools/mock_ollama_v0_4.sh \
    cargo run -q --manifest-path adl/Cargo.toml --bin adl -- "$EXAMPLE" --run --trace --allow-unsigned
...
run_tamper() {
  ...
  cargo test -q --manifest-path adl/Cargo.toml --lib remote_exec::tests::security_envelope_rejects_tampered_signed_request -- --exact --nocapture
}
