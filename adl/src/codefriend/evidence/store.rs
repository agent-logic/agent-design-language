...
impl Store {
    ...
    /// Retain an already validated remote admission without changing its identity
    /// or extending its lifetime. The caller must independently authorize its source.
    pub(crate) fn retain_original(&self, admission: &Admission) -> Result<()> {
        admission.validate()?;
