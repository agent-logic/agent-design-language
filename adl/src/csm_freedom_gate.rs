...
pub fn api_status(artifact: &Value, runtime_capability: Value, agent_instance_id: &str) -> Value {
    ...
    json!({
        ...
        "executor_requires_gate_decision": value.get("executor_requires_gate_decision").cloned().unwrap_or(json!(true)),
        "unmediated_execution_allowed": value.get("unmediated_execution_allowed").cloned().unwrap_or(json!(false)),
