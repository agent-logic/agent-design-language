...
fn red_blue_scenarios() -> Result<Vec<RedBlueScenario>> {
    ...
    Ok(vec![
        scenario(
            "malformed_snapshot",
            "tamper continuity capsule manifest and snapshot segment",
            "red_team_fixtures.json#malformed_snapshot",
            "capsule_stage_rejected",
            "csm_security_event.snapshot_integrity_refused",
            "refused",
            "cav_red_blue_events.jsonl#malformed_snapshot",
            malformed,
            false,
        ),
        scenario(
            "unauthorized_control_command",
            "request resume while invariant evidence is blocking",
            "runtime_v2/security_boundary/proof_packet.json",
            "resume_refused",
            "csm_security_event.operator_command_refused",
            "refused",
            "cav_red_blue_events.jsonl#unauthorized_control_command",
            unauthorized,
            true,
        ),
        scenario(
            ...
            "red_team_fixtures.json#telemetry_injection",
            "telemetry_payload_sanitized",
            "csm_security_event.telemetry_injection_detected",
            "detected",
            "cav_red_blue_events.jsonl#telemetry_injection",
            telemetry,
            false,
        ),
        scenario(
            "credential_path_leakage",
            "add api_key and host-private path fields to retained artifact",
            "red_team_fixtures.json#credential_path_leakage",
            "artifact_redaction_refused",
            "csm_security_event.credential_path_leakage_refused",
            "refused",
            "cav_red_blue_events.jsonl#credential_path_leakage",
            leakage,
            false,
        ),
        scenario(
            "replay_tampering",
            "change replay manifest hash and DAG lineage reference",
            "red_team_fixtures.json#replay_tampering",
            "replay_guard_rejected",
            "csm_security_event.replay_tampering_refused",
            "refused",
            "cav_red_blue_events.jsonl#replay_tampering",
            replay,
            false,
        ),
        scenario(
            ...
            "red_team_fixtures.json#cloud_hook_denial",
            "cloud_hook_denied_locally",
            "csm_security_event.cloud_hook_denial",
            "refused",
            "cav_red_blue_events.jsonl#cloud_hook_denial",
            cloud,
            false,
        ),
    ])
...
fn scenario(
    ...
    RedBlueScenario {
        id: id.to_string(),
        red_team_attempt: red.to_string(),
        fixture_ref: fixture_ref.to_string(),
        integrated_csm_path: true,
        expected_detection: expected.to_string(),
        observed_event: observed.to_string(),
