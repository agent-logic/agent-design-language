...
fn card_lifecycle_blocks_bootstrap_stp_before_execution() {
    ...
    assert_eq!(lifecycle.pr_run_readiness, "blocked");
    assert_stage(&lifecycle, "STP", "scaffold", false, false);
    assert_eq!(
        lifecycle
            .stages
            .iter()
            .find(|stage| stage.stage == "STP")
...
#[test]
fn preflight_card_readiness_blocks_bootstrap_source_prompt_before_execution() {
    let repo = lifecycle_temp_repo("preflight-bootstrap-source");
    let issue_ref = IssueRef::new(
        4487,
        "v0.91.6".to_string(),
        "worktree-safe-truth".to_string(),
    ...
    .expect("write bootstrap source prompt");
    let bundle = issue_ref.task_bundle_dir_path(&repo);
    fs::create_dir_all(&bundle).expect("create bundle");
    fs::write(
        issue_ref.task_bundle_input_path(&repo),
    ...
    fs::write(
        issue_ref.task_bundle_validation_plan_path(&repo),
        "---\nartifact_type: \"structured_validation_planning_prompt\"\nbranch: \"not bound yet\"\ncard_status: \"ready\"\nstatus: \"reviewed\"\ninitial_pvf_lane: \"tooling\"\nplanned_pvf_lane: \"tooling\"\nplanned_validation_seconds: \"30\"\nplanned_validation_tokens: \"800\"\nvalidation_commands:\n  - \"cargo test --manifest-path adl/Cargo.toml preflight_card_readiness_blocks_bootstrap_source_prompt_before_execution -- --nocapture\"\n---\n\n# Validation Planning Prompt\n\n## Validation Commands\n\n- cargo test --manifest-path adl/Cargo.toml preflight_card_readiness_blocks_bootstrap_source_prompt_before_execution -- --nocapture\n",
    )
    .expect("write vpp");
    fs::write(
        issue_ref.task_bundle_review_policy_path(&repo),
...
#[test]
fn preflight_card_readiness_blocks_bootstrap_stp_before_execution() {
    let repo = lifecycle_temp_repo("preflight-bootstrap-stp");
    let issue_ref = IssueRef::new(
        4487,
        "v0.91.6".to_string(),
        "worktree-safe-truth".to_string(),
    ...
    fs::write(
        &source,
        "# Fixture\n\n## Summary\n\nAuthored source prompt.\n\n## Goal\n\nBlock lifecycle execution when STP is still generated text.\n\n## Acceptance Criteria\n\n- preflight blocks bootstrap STP residue\n",
    )
    .expect("write authored source prompt");
    let bundle = issue_ref.task_bundle_dir_path(&repo);
    fs::create_dir_all(&bundle).expect("create bundle");
    fs::write(
        issue_ref.task_bundle_input_path(&repo),
    ...
    fs::write(
        issue_ref.task_bundle_validation_plan_path(&repo),
        "---\nartifact_type: \"structured_validation_planning_prompt\"\nbranch: \"not bound yet\"\ncard_status: \"ready\"\nstatus: \"reviewed\"\ninitial_pvf_lane: \"tooling\"\nplanned_pvf_lane: \"tooling\"\nplanned_validation_seconds: \"30\"\nplanned_validation_tokens: \"800\"\nvalidation_commands:\n  - \"cargo test --manifest-path adl/Cargo.toml preflight_card_readiness_blocks_bootstrap_stp_before_execution -- --nocapture\"\n---\n\n# Validation Planning Prompt\n\n## Validation Commands\n\n- cargo test --manifest-path adl/Cargo.toml preflight_card_readiness_blocks_bootstrap_stp_before_execution -- --nocapture\n",
    )
    .expect("write vpp");
    fs::write(
        issue_ref.task_bundle_review_policy_path(&repo),
