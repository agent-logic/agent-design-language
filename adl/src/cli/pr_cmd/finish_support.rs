...
pub(super) fn select_finish_validation_plan_for_finish_with_release_gate_disposition(
    ...
    if finish_issue_needs_wp08_acip_sns_validation(issue_number, changed_paths) {
        return Ok(build_wp08_acip_sns_validation_plan());
    }
    if finish_issue_needs_wp08_local_polis_ssm_validation(issue_number, changed_paths) {
        return Ok(build_wp08_local_polis_ssm_validation_plan());
    }
...
fn finish_issue_needs_wp08_acip_sns_validation(
    issue_number: u32,
    changed_paths: &[String],
) -> bool {
    if issue_number != 4685 {
        return false;
    }
    !changed_paths.is_empty()
        && changed_paths.iter().all(|path| {
            ...
            matches!(
                ...
                "adl/Cargo.lock"
                    ...
                    | "adl/tools/test_run_wp08_acip_sns_live_proof.sh"
                    | "adl/tools/validate_v0917_soak2_4682_status.sh"
                    | "adl/tools/validate_wp08_acip_sns_live_proof.py"
                    | "docs/tooling/RUNTIME_AWS_ACIP_SNS.md"
                    | "docs/milestones/v0.91.7/review/runtime/soak2_4682/blocker_register.json"
                    | "docs/milestones/v0.91.7/review/runtime/soak2_4682/soak2_execution_status_4682.json"
            ) || trimmed.starts_with("docs/milestones/v0.91.7/review/runtime/wp08_acip_sns_4685/")
        })
}

fn build_wp08_acip_sns_validation_plan() -> FinishValidationPlan {
    let mut commands = vec![
        "bash adl/tools/check_no_tracked_adl_issue_record_residue.sh".to_string(),
        "git diff --check".to_string(),
    ];
...
fn finish_issue_needs_wp08_local_polis_ssm_validation(
    issue_number: u32,
    changed_paths: &[String],
) -> bool {
    if issue_number != 4687 {
        return false;
    }
    !changed_paths.is_empty()
        && changed_paths.iter().all(|path| {
            ...
            matches!(
                ...
                "adl/config/validation_lane_selector.v0.91.6.json"
                    | "adl/src/cli/pr_cmd/finish_support.rs"
                    | "adl/src/cli/tests/pr_cmd_inline/finish/arg_render.rs"
                    | "adl/tools/run_wp08_local_polis_ssm_proof.py"
                    | "adl/tools/run_wp08_local_polis_ssm_proof.sh"
                    | "adl/tools/test_run_wp08_local_polis_ssm_proof.sh"
                    | "adl/tools/validate_wp08_local_polis_ssm_proof.py"
                    | "docs/tooling/LOCAL_POLIS_SSM_OPERATIONS.md"
            ) || trimmed
                .starts_with("docs/milestones/v0.91.7/review/runtime/wp08_local_polis_ssm_4687/")
        })
}

fn build_wp08_local_polis_ssm_validation_plan() -> FinishValidationPlan {
    let mut commands = vec![
        "bash adl/tools/check_no_tracked_adl_issue_record_residue.sh".to_string(),
        "git diff --check".to_string(),
    ];
    ...
    push_finish_validation_command(
        &mut commands,
        "bash adl/tools/test_run_wp08_local_polis_ssm_proof.sh",
    );
    push_finish_validation_command(
        &mut commands,
...
fn finish_issue_needs_wp08_s3_obsmem_archive_policy_validation(
    issue_number: u32,
    changed_paths: &[String],
) -> bool {
    if issue_number != 4688 {
        return false;
    }
    !changed_paths.is_empty()
        && changed_paths.iter().all(|path| {
            ...
            matches!(
                ...
                "adl/config/validation_lane_selector.v0.91.6.json"
                    | "adl/src/cli/pr_cmd/finish_support.rs"
                    | "adl/src/cli/tests/pr_cmd_inline/finish/arg_render.rs"
                    | "adl/tools/setup_wp08_s3_obsmem_archive_policy.sh"
                    | "adl/tools/test_setup_wp08_s3_obsmem_archive_policy.sh"
                    | "adl/tools/validate_wp08_s3_obsmem_archive_policy.py"
                    | "docs/tooling/S3_OBSMEM_COMMUNITY_MEMORY_ARCHIVE_POLICY.md"
            ) || trimmed
                .starts_with("docs/milestones/v0.91.7/review/runtime/wp08_s3_obsmem_archive_4688/")
        })
}

fn build_wp08_s3_obsmem_archive_policy_validation_plan() -> FinishValidationPlan {
    let mut commands = vec![
        "bash adl/tools/check_no_tracked_adl_issue_record_residue.sh".to_string(),
        "git diff --check".to_string(),
    ];
    ...
    push_finish_validation_command(
        &mut commands,
        "bash adl/tools/test_setup_wp08_s3_obsmem_archive_policy.sh",
    );
    push_finish_validation_command(
        &mut commands,
...
fn finish_issue_needs_wp08_aws_signal_integration_validation(
    issue_number: u32,
    changed_paths: &[String],
) -> bool {
    if issue_number != 4686 {
        return false;
    }
    !changed_paths.is_empty()
        && changed_paths.iter().all(|path| {
...
fn build_wp08_aws_signal_integration_validation_plan() -> FinishValidationPlan {
    let mut commands = vec![
        "bash adl/tools/check_no_tracked_adl_issue_record_residue.sh".to_string(),
        "git diff --check".to_string(),
    ];
    ...
    push_finish_validation_command(
        &mut commands,
        "bash adl/tools/test_run_wp08_aws_signal_integration_live_proof.sh",
    );
    push_finish_validation_command(
        &mut commands,
    ...
    push_finish_validation_command(
        &mut commands,
        "bash adl/tools/validate_v0917_no_sparrow_4909_status.sh",
    );
    push_finish_validation_command(
        &mut commands,
        "bash adl/tools/validate_v0917_soak2_4682_status.sh",
    ...
    if matches!(
        ...
        for command in &plan.commands {
            ...
            match command.as_str() {
                ...
                "cargo test --manifest-path adl/Cargo.toml --bin adl-pr-finish cli::pr_cmd::tests::finish::arg_render::finish_validation_profile_classifies_wp08_s3_obsmem_archive_policy_slice -- --exact --nocapture" => {
                    run_finish_validation_status(
                        ...
                        &[
                            ...
                            "--bin",
                            "adl-pr-finish",
                            "cli::pr_cmd::tests::finish::arg_render::finish_validation_profile_classifies_wp08_s3_obsmem_archive_policy_slice",
                            "--",
                            "--exact",
                            "--nocapture",
                        ],
                ...
                "cargo test --manifest-path adl/Cargo.toml --bin adl-pr-finish cli::pr_cmd::tests::finish::arg_render::finish_validation_profile_classifies_wp08_acip_sns_slice -- --exact --nocapture" => {
                    run_finish_validation_status(
                        ...
                        &[
                            ...
                            "--bin",
                            "adl-pr-finish",
                            "cli::pr_cmd::tests::finish::arg_render::finish_validation_profile_classifies_wp08_acip_sns_slice",
                            "--",
                            "--exact",
                            "--nocapture",
                        ],
                ...
                "cargo test --manifest-path adl/Cargo.toml --bin adl-pr-finish cli::pr_cmd::tests::finish::arg_render::finish_validation_profile_classifies_wp08_local_polis_ssm_slice -- --exact --nocapture" => {
                    run_finish_validation_status(
                        ...
                        &[
                            ...
                            "--bin",
                            "adl-pr-finish",
                            "cli::pr_cmd::tests::finish::arg_render::finish_validation_profile_classifies_wp08_local_polis_ssm_slice",
                            "--",
                            "--exact",
                            "--nocapture",
                        ],
                ...
                "cargo test --manifest-path adl/Cargo.toml --bin adl-pr-finish cli::pr_cmd::tests::finish::arg_render::finish_validation_profile_classifies_wp08_aws_signal_integration_slice -- --exact --nocapture" => {
                    run_finish_validation_status(
                        ...
                        &[
                            ...
                            "--bin",
                            "adl-pr-finish",
                            "cli::pr_cmd::tests::finish::arg_render::finish_validation_profile_classifies_wp08_aws_signal_integration_slice",
                            "--",
                            "--exact",
                            "--nocapture",
                        ],
                ...
                "bash adl/tools/test_run_wp08_acip_sns_live_proof.sh" => {
                    let script = repo_root.join("adl/tools/test_run_wp08_acip_sns_live_proof.sh");
                    run_finish_validation_status("bash", &[path_str(&script)?])?;
                }
                "bash adl/tools/test_run_wp08_local_polis_ssm_proof.sh" => {
                    let script =
                        repo_root.join("adl/tools/test_run_wp08_local_polis_ssm_proof.sh");
                    run_finish_validation_status("bash", &[path_str(&script)?])?;
                }
                "bash adl/tools/test_setup_wp08_s3_obsmem_archive_policy.sh" => {
                    let script =
                        repo_root.join("adl/tools/test_setup_wp08_s3_obsmem_archive_policy.sh");
                    run_finish_validation_status("bash", &[path_str(&script)?])?;
                }
                "bash adl/tools/test_run_wp08_aws_signal_integration_live_proof.sh" => {
                    let script =
                ...
                "python3 adl/tools/validate_wp08_acip_sns_live_proof.py docs/milestones/v0.91.7/review/runtime/wp08_acip_sns_4685/acip_sns_summary.json docs/milestones/v0.91.7/review/runtime/wp08_acip_sns_4685/sns_resource_summary.json" => {
                    let script =
                        repo_root.join("adl/tools/validate_wp08_acip_sns_live_proof.py");
                    let summary = repo_root.join(
                        "docs/milestones/v0.91.7/review/runtime/wp08_acip_sns_4685/acip_sns_summary.json",
                    );
                    let resource_summary = repo_root.join(
                    ...
                    run_finish_validation_status(
                        ...
                        &[
                            path_str(&script)?,
                            path_str(&summary)?,
                            path_str(&resource_summary)?,
                        ],
                    )?;
                }
                "python3 adl/tools/validate_wp08_local_polis_ssm_proof.py docs/milestones/v0.91.7/review/runtime/wp08_local_polis_ssm_4687/local_polis_ssm_summary.json" => {
                    let script =
                        repo_root.join("adl/tools/validate_wp08_local_polis_ssm_proof.py");
                    let summary = repo_root.join(
                ...
                "python3 adl/tools/validate_wp08_s3_obsmem_archive_policy.py docs/milestones/v0.91.7/review/runtime/wp08_s3_obsmem_archive_4688/archive_policy_summary.json" => {
                    let script =
                        repo_root.join("adl/tools/validate_wp08_s3_obsmem_archive_policy.py");
                    let summary = repo_root.join(
